...
1. **前端不暴露 batch**：用户在前端选择的是一个**绝对输出目录** ``output_dir``，
   训练与分析的产物固定落在::
       ...
       <output_dir>/results/     ← 实验目录、summary、analyse_out
...
class PipelineContext:
    ...
    def results_base(self) -> Path:
        """训练结果的根目录（绝对）：<output_dir>/results。"""
    ...
    def data_path(self) -> Path:
        return _abs(self.data_dir or (self.repo_root / "data" / "proceeded_data"), self.repo_root)
...
def _cmd_feature_engineering(ctx: PipelineContext) -> List[str]:
    return [ctx.python, _script(ctx, "src/feature_engineering.py"),
...
def _cmd_train_grid(ctx: PipelineContext) -> List[str]:
    cmd = [ctx.python, _script(ctx, "data_digging.py"),
...
def _cmd_generate_candidates(ctx: PipelineContext) -> List[str]:
    cmd = [ctx.python, _script(ctx, "predict.py"), "--generate-candidates",
           "--data-dir", str(ctx.data_path),
           "--results-dir", str(ctx.results_base)]
    if ctx.batch_name:
        cmd += ["--batch-name", str(ctx.batch_name)]
...
def _cmd_collect_results(ctx: PipelineContext) -> List[str]:
    cmd = [ctx.python, _script(ctx, "analyse/collect_results.py"),
...
def _cmd_anomaly_treatment(ctx: PipelineContext) -> List[str]:
    return [ctx.python, _script(ctx, "analyse/anomaly_treatment.py"),
            "--batch-dir", str(ctx.results_path)]


def _cmd_importance_extraction(ctx: PipelineContext) -> List[str]:
    return [ctx.python, _script(ctx, "analyse/importance_extraction.py"),
            "--batch_dir", str(ctx.results_path)]


def _cmd_legacy_visualization(ctx: PipelineContext) -> List[str]:
    cmd = [ctx.python, _script(ctx, "analyse/visualization.py"),
           "--batch-dir", str(ctx.results_path)]
...
def _cmd_analysis_engine(ctx: PipelineContext) -> List[str]:
    cmd = [ctx.python, "-m", "analyse.pipeline", "--batch-dir", str(ctx.results_path)]
...
STEPS: Tuple[StepSpec, ...] = (
    StepSpec(step_id="feature_engineering", title="特征工程（23×8 张量 + schema）",
             ...
             cli_script="src/feature_engineering.py"),
    StepSpec(step_id="train_grid", title="受控网格训练（data_digging.py）",
             ...
             description="多模型 × 多环境 × 多划分 × 多种子受控实验；权重写入 <输出目录>/models，日志写入 <输出目录>/logs。",
             cli_script="data_digging.py"),
    ...
    StepSpec(step_id="collect_results", title="指标汇总（collect_results.py）",
             ...
             cli_script="analyse/collect_results.py"),
    StepSpec(step_id="anomaly_treatment", title="异常实验检测（anomaly_treatment.py）",
             ...
             cli_script="analyse/anomaly_treatment.py"),
    StepSpec(step_id="importance_extraction", title="关键调控特征库（importance_extraction.py）",
             ...
             cli_script="analyse/importance_extraction.py"),
    StepSpec(step_id="legacy_visualization", title="全景图（visualization.py）",
             category=CATEGORY_ANALYSIS, heavy=False, requires=("train_grid",),
             artifacts=("summary/plots",),
             description="23 nt 位置热图、环境增量树、表观因子对比等全景图。",
             cli_script="analyse/visualization.py"),
    StepSpec(step_id="analysis_engine", title="分析引擎（analyse.pipeline：统计/证据/报告/图表）",
             category=CATEGORY_ANALYSIS, heavy=False, requires=("train_grid",),
             artifacts=("analyse_out/analysis_status.json", "analyse_out/summary", "analyse_out/figures"),
             description="配对 bootstrap、置换、BH-FDR、区组 ANOVA、motif 发现、证据整合与报告。",
             cli_script="analyse/pipeline.py"),
