...
用法:
    python predict.py --data-dir data/proceeded_data --models linear xgboost \\
...
def _load_mixed_subset(data_dir: str, schema: Dict, plan: Dict,
    ...
    from src.input_control.cell_line_division import (
...
def _build_torch_model(kind: str, input_dim_2d: int, input_dim_3d_channels: int,
    ...
    if kind == "mlp":
        from src.mlp.mlp import MLPModel
        model = MLPModel(
    ...
    if kind.startswith("cnn"):
        ...
        from src.cnn.cnn import CNNModel
        model = CNNModel(
...
def _drop_lr_reference_columns(X2: np.ndarray, feature_names: List[str]):
    ...
    from src.linear_regression.linear_regression import select_non_t_reference_features
...
def main():
    args = parse_args()
    from src.input_control.cell_line_division import load_feature_schema
