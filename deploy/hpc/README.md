...
本批次采用 **group-aware 无泄漏划分**（见 `docs/HPC_RERUN_PREFLIGHT.md`）。

**超算只负责训练出数据**（1344 个 run 的指标/预测/重要性/元数据）；
所有分析、统计检验、图表、论文资产都在本地用仓库的 `analysis/` + `paper/` 完成。
因此本包**不含** `analysis/`、`paper/`、`app/backend/`、`app/frontend/`。
...
requirements_hpc.txt        本平台(CentOS7/glibc2.17/8×A100/驱动550)环境清单 ← 按这个装
requirements_frozen.txt     开发机审计栈（用于比对差异, 见 docs/HPC_ENVIRONMENT_FIT.md）
docs/HPC_ENVIRONMENT_FIT.md 平台适配说明（cu124、conda-forge、多卡绑定、等价性验证）
docs/HPC_ENV_COMPATIBILITY_REPORT.md ★实测报告：你的栈能不能跑 + 实测数值偏移
docs/HPC_RERUN_PREFLIGHT.md 自检报告（含证据、限制、验收标准）
```
...
#    python -c "import xgboost,shap;print(xgboost.__version__, shap.__version__)"   # 必须都有
...
# 2) 全量重跑
WORKERS=8 bash run.sh
#   bash run.sh single | bash run.sh all | bash run.sh mixed   # 分片跑
...
| 约束 | 原因 |
|---|---|
| **不要**改 `BATCH` 为 `batch_20260909_full` 或任何已存在的非空目录 | 已存在的 run 会被判定「已完成」而跳过，新旧（泄漏/无泄漏）结果混批 |
