...
def _ensure_repo_root_on_path() -> str:
    """让 `python analysis/panorama.py` 也能 import `analysis.*`。

    直接以脚本方式运行本文件时, sys.path[0] 是 analysis/ 目录, 仓库根不在路径上,
    于是 `import analysis.config` / `analysis.environment.factorial_dag` 会失败。
    这里把仓库根 (本文件的父目录的父目录) 插到 sys.path, 幂等且无副作用。
    通过 analysis.visualization 包导入 (backend_runner) 时该路径已存在, 不会有影响。
    """
...
MODEL_METRIC_CONFIG = {
    ...
    "mlp": {
        "col_candidates": ["MLP_IG", "ig_mean", "ig_snr"],
...
def parse_feature_position_channel(feat_name: str) -> Tuple[int, str]:
    ...
    # F1 修复: 直接内联 canonical 解析规则（避免对 analysis.attribution 的导入依赖）：
    #   'pos12_A'  风格 → 数字已是 1-based
    #   'A_pos_12' 风格 → 数字是 0-based 索引，需 +1
    # 此前对所有 <=22 的值一律 +1，使 linear/xgboost/mlp 的热图位置变成 2..23（缺 1）。
    mm = re.search(r'pos_?(\d+)', fn, re.IGNORECASE)
    if mm and not ('bias' in fn.lower() or 'intercept' in fn.lower()):
        pos0 = int(mm.group(1))
        one_based = mm.start() == 0 and fn.lower().startswith('pos')
...
def generate_all_visualizations(batch_dir_str: str, plots_dir_str: Optional[str] = None):
    ...
    try:
        _ensure_repo_root_on_path()
        from analysis.environment.factorial_dag import generate_environment_factorial_dag
        # 2026-09-13: 本模块是"全景图"步骤, 产出的环境表是**画图用的中间件**。
        # 分析引擎 (analysis.pipeline) 现在占用了 <batch>/summary/tables/,
        # 两者会写同名文件 (environment_{nodes,edges,interactions,ablation,dag_report}.csv),
        # 且内容口径不同 (引擎版含 bootstrap CI 列)。为避免互相覆盖,
        # 全景图把自己的中间表放在 plots 目录下, 不再写入 summary/tables/。
        env_tables_dir = plots_root / "_environment_tables"
        dag_res = generate_environment_factorial_dag(
            batch_dir, figures_dir=plots_root / "03_environment",
            tables_dir=env_tables_dir,
            summary_dir=env_tables_dir)
        print(f"        tables: {Path(dag_res['paths']['nodes']).name}, "
              f"{Path(dag_res['paths']['edges']).name}; "
              f"DAG 图 {len(dag_res['dag_figures'])} 张 -> {env_tables_dir}")
...
def generate_env_increment_trees(batch_dir: Path, output_dir: Path):
    ...
    主视图请用 analysis.environment.factorial_dag.generate_environment_factorial_dag。
    """
