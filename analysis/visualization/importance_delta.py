"""analysis.visualization.importance_delta — Importance/Attribution–ΔR² 二维证据坐标图。

X = ΔR² (配对基线: 同 split/cell/model/seed cohort, 由 analysis.environment.incremental_effect 计算)
...
from analysis.importance_metrics import (ENV_FACTORS, PRIMARY_METRICS, evidence_strength,
...
def scan_factor_importance(batch_dir: str | Path) -> pd.DataFrame:
    ...
    """
    from analysis.importance_extraction import identify_feature_channel, normalize_cell_line, parse_info_file
...
def write_report_md(table: pd.DataFrame, path: str | Path, filter_mode: str,
    ...
    lines = [
        ...
        "由 `analysis.environment.incremental_effect` 计算；本图对同一 factor 跨背景取均值。", "",
        "## 3. Y 轴定义",
...
from analysis.importance_metrics import NOT_USED_AS_Y as NOT_USED_AS_Y_ROWS  # noqa: E402
...
def run_importance_delta_analysis(batch_dir: str | Path, output_dir: str | Path,
    ...
    out = Path(output_dir)
    # 2026-09-13: 产物根改为 <batch>/summary, 报告子目录用 reports/ 避免 summary/summary
    tables = out / "tables"; summary = out / "reports"; figures = out / "figures"
