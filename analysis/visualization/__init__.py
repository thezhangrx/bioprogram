...
from analysis.visualization import (attribution_plots, cellline_plots,  # noqa: E402
...
# ---------------------------------------------------------------------------
# 兼容桥 (迁移期): 单文件全景图入口已更名 analysis/panorama.py, 避免与证据图包同名冲突。
# 包目录会遮蔽同目录下的 .py 模块, 导致训练流程 backend_runner Step 6 的
# `from analysis.visualization import generate_all_visualizations` 失效。
# 用 PEP 562 __getattr__ 惰性加载旧模块文件并透传其函数 —— 不改训练侧代码。
# 新代码一律用 render_all; 旧入口仅作兼容, 随迁移完成移除。
# ---------------------------------------------------------------------------
_LEGACY_VIZ_FILE = Path(__file__).resolve().parent.parent / "panorama.py"
...
def _load_legacy_viz_module():
    ...
    if _legacy_viz_module is None:
        ...
        spec = importlib.util.spec_from_file_location("analysis._legacy_viz_module",
                                                      _LEGACY_VIZ_FILE)
