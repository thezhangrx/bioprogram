...
Run:  python analysis/reporting/paper/make_assets.py [--batch ultimate_run]
Inputs are READ-ONLY artifacts under results/batches/<batch>/summary (never modified).
Every number written here is recomputed from those artifacts (no hard-coded results).

权威批次 = ultimate_run（group-aware 无泄漏，1344 runs，验收 PASS）。
旧批次 batch_20260909_full 存在已确认的划分泄漏，**不得**用于论文数字。
"""
...
ANALYSIS = ROOT / "results" / "tables" / "paper"
...
def figure2_prediction(t: pd.DataFrame, loco: pd.DataFrame) -> None:
    ...
    for i, m in enumerate(["xgboost", "transformer", "cnn(7|3)", "mlp"]):
        # D6 之后 loco_performance 使用 canonical numerical-validity contract：
        # 列为 R2_mean / R2_median（发散 run 已隔离并给出 n_valid），此处取 median。
        vals = [float(sub[(sub.model == m) & (sub.cell_line == c)]["R2_median"].iloc[0])
...
def write_tables(seqs: pd.DataFrame, pred: pd.DataFrame, env_cross: pd.DataFrame,
    ...
    # Table 2 prediction（F2 修复：直接从 canonical experiment-level 表聚合）
    # 此前用 prediction_summary（每 model×split 仅 1 行）再取中位 → median≡mean、SD=NA、
    # diverged 只计 1、linear 发散值未被剔除。现改为对**实验**聚合：
    #   n_experiments = 该 (model, split) 的 run 数（剔除发散前）
    #   R2_median / R2_mean / R2_sd / RMSE_median / MAE_median / Spearman_median 均基于**有效 run**
    #   diverged = |R²| >= UNSTABLE 的 run 数（不删除，仅隔离）
    order = ["cnn(3|3)", "cnn(5|3)", "cnn(7|3)", "mlp", "transformer", "xgboost", "linear"]
    exp = t if "t" in dir() else pd.read_csv(BASE / "tables" / "experiment_table.csv")
    exp = pd.read_csv(BASE / "tables" / "experiment_table.csv")
    rows = []
    for m in order:
        for split in ("single", "mixed", "all"):
            sub = exp[(exp.model == m) & (exp.split_type == split)]
            if sub.empty:
                continue
            r2 = pd.to_numeric(sub["R2"], errors="coerce")
            valid = sub[r2.abs() < UNSTABLE]
            if valid.empty:
                continue
            rows.append({"model": m, "split": split, "n_experiments": int(len(sub)),
                         "n_valid": int(len(valid)),
                         "R2_median": valid["R2"].median(), "R2_mean": valid["R2"].mean(),
                         "R2_sd": valid["R2"].std(ddof=1),
                         "RMSE_median": valid["RMSE"].median(),
                         "MAE_median": valid["MAE"].median(),
                         "Spearman_median": valid["Spearman"].median(),
                         "diverged": int((r2.abs() >= UNSTABLE).sum())})
    t2 = pd.DataFrame(rows)
    _tex_table(t2, "tab2_prediction",
               caption="Held-out test performance aggregated over experiments (canonical experiment-level "
                       "table). n_valid is the number of non-divergent runs used for the statistics; "
                       "diverged counts runs with $|R^{2}| \\ge 10$ (isolated, not used in the statistics, not "
                       "deleted from the raw artifacts).",
    ...
    for f in ["ctcf", "dnase", "h3k4me3", "rrbs"]:
        ...
        rows.append({
            "factor": f.upper(),
            # R10: Tier 与各判据解耦, 便于读者核对"为何是该 Tier"
            "mean_dR2": e["overall_effect"],
            "median_dR2": e.get("median_effect"),
            "effect_gate": ("pass" if bool(e.get("effect_gate_pass")) else "fail"),
            "n_models": int(e.get("n_models") or 0),
            "direction_conflict": ("yes" if bool(e.get("model_direction_conflict")) else "no"),
            "min_edge_FDR": e.get("min_edge_fdr"),
            "n_contexts": int(e.get("n_contexts_after_dedup") or 0),
            "selected_context": e.get("selected_context"),
            "fwer_upper_bound": e.get("fwer_upper_bound"),
            "concordance": e["direction_concordance"],
            "mlevel_bootstrap": f"[{fci_row['ci_low']:.4f}, {fci_row['ci_high']:.4f}]",
            "ci_excludes_zero": bool(fci_row["ci_low"] > 0 or fci_row["ci_high"] < 0),
            "anova_F": an["F_statistic"], "anova_p": an["p_value"],
            "cell_line_context": e["cell_line_consistency"],
    ...
    _tex_table(t3, "tab3_environment",
               caption="Environmental factors: cross-model incremental predictive value "
                       ...
                       "|ΔR²| > 10), model-level bootstrap interval (n = 7 model configurations), paired per-sample edge "
                       "bootstrap (CI excluding zero), permutation-based FDR, blocked factorial ANOVA "
                       "and cell-line context label. Effect gate = $|$model-equal-weight mean $\\Delta R^{2}| \\ge 0.01$; min_edge_FDR = Minimal Edge-level FDR across Tested Contexts (existence-oriented, extremum selection, not a factor-level FDR); n_contexts = de-duplicated tested contexts; mlevel_bootstrap = Model-Level Bootstrap Interval (n = 7 model configurations).",
    ...
    # Table S/methods: analysis thresholds summary (values read from analysis/config.py)
    ...
    mt = pd.DataFrame([
        ...
        {"item": "evidence tier: min |delta R2| gain gate",
         "value": f"{cfg.evidence.min_absolute_delta_r2}"},
        {"item": "evidence tier: permutation FDR max / min bootstrap iter",
         "value": f"{cfg.statistical.fdr_weak}/{cfg.evidence.min_bootstrap_iterations}"},
        {"item": "unstable effect threshold (all analyses)",
         "value": f"{cfg.consensus.unstable_effect_threshold}"},
        ...
        {"item": "Importance-deltaR2 only: SNR (strong/mod./weak)",
         "value": f"{cfg.attribution.snr_threshold}/{cfg.attribution.snr_moderate}/{cfg.attribution.snr_weak}"},
        {"item": "Importance-deltaR2 only: min effect size / FDR (strong/mod.)",
         "value": f"{cfg.attribution.min_effect_size}/{cfg.statistical.fdr_strong}/{cfg.statistical.fdr_moderate}"},
    ])
    _tex_table(mt, "tabS1_methods",
               caption="Analysis thresholds used in this study; all values are read from the central "
                       "analysis configuration file rather than hard-coded in analysis code. Rows "
                       "labelled 'Importance-deltaR2 only' feed the supplementary Importance--deltaR2 "
                       "annotation and do NOT enter the evidence-tier rule; the authoritative tier rule "
                       "is classify_evidence_tier() in analysis/evidence/integration.py.",
...
def main() -> None:
    ...
    # D4 修复: environment_edges/environment_bootstrap 中 mixed 行有重复（859 行 vs 唯一 224），
    # 必须先按 canonical experiment identity 去重，否则分母被放大（2651 vs 2016）。
    eb = eb.drop_duplicates(subset=key, keep="first")
    sys.path.insert(0, str(ROOT))
    from analysis.environment.incremental_effect import parse_environment_set
    # 每个 lattice edge 可能有多 seed; 先按 edge 聚合 (conservative union CI), 再与 unique edge 表合并。
    agg = boot.groupby(key).agg(estimate=("estimate", "mean"),  # 每 edge 跨 seed 先聚合（原有口径）
