...
class TrainingConfig:
    ...
    def default_dirs(self) -> None:
        ...
        if not self.results_dir:
            self.results_dir = str(repo / "results" / "batches")
        if not self.model_dir:
            self.model_dir = str(repo / "models" / "weights")
        if not self.logs_dir:
            self.logs_dir = str(repo / "results" / "logs")
...
class LocalRuntime:
    ...
    def submit_raw(self, cmd: List[str], kind: str, batch_name: str = "",
        ...
        训练与流程步骤（feature_engineering / collect_results / analysis.pipeline ...）共用，
