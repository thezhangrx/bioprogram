"""流程步骤服务：共享编排层 ``workflows.orchestrator.steps`` 的只读列表 + 受控执行。

设计要点
--------
* **唯一定义源**：步骤、命令、依赖、产物全部来自 ``workflows/orchestrator/steps.py``（向导与网页共用），
  本模块不复制任何命令拼装逻辑。
...
if str(_REPO) not in sys.path:          # 共享层 workflows/ 位于项目根目录
    sys.path.insert(0, str(_REPO))

from workflows.orchestrator import (  # noqa: E402  (路径注入后再导入)
