 ...
 */
export function DirPicker({ open, initial, title = "选择目录", mode = "dir", onPick, onClose }:
  {
    open: boolean;
    initial?: string;
    title?: string;
    /** dir = 只能选目录；file = 只能选数据文件；any = 两者皆可 */
    mode?: "dir" | "file" | "any";
  ...
  useEffect(() => {
    if (!open) return;
    fsApi.list(cur, mode !== "dir").then((l) => { setListing(l); setCur(l.path); setErr(null); })
      ...
      <div className="flex max-h-[80vh] w-[720px] flex-col rounded-xl bg-white shadow-xl">
          ...
          {(listing?.entries ?? []).map((e) => (
            <li key={e.path} className="flex items-center gap-1">
              <button onClick={() => setCur(e.path)}
                      className="flex flex-1 items-center gap-2 rounded px-2 py-1 text-left text-[12px] hover:bg-slate-50">
                <span>{e.is_dir ? "📁" : "📄"}</span>
                <span className="flex-1 truncate">{e.name}</span>
                {e.is_dir && e.is_empty && <span className="text-[10px] text-slate-300">空</span>}
                {!e.is_dir && e.size !== undefined && (
                  <span className="text-[10px] text-slate-400">{Math.max(1, Math.round(e.size / 1024))} KB</span>
                )}
              </button>
              {!e.is_dir && mode !== "dir" && (
                <button onClick={() => onPick(e.path)}
        ...
        <div className="flex flex-wrap items-center gap-2 border-t border-slate-100 px-4 py-2 text-[11px]">
          ...
          {mode !== "file" ? (
            ...
            <span className="text-slate-400">请选择下方数据文件（.csv/.tsv）</span>
          )}
        </div>
      </div>
    </div>
