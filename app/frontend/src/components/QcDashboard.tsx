...
function overall(sum: Record<string, unknown>): { label: string; cls: string } {
  ...
  if ((amb ?? 0) > 0 || (out ?? 0) > 0) {
    return { label: "🟡 建议复核", cls: "text-amber-700" };
  }
  const pass = sum["qc_pass"];
  if (pass === false) return { label: "🔴 发现严重问题", cls: "text-red-600" };
  if (pass === true) return { label: "🟢 良好", cls: "text-emerald-600" };
  return { label: "已完成 — 请查看下方明细", cls: "text-slate-600" };
}
...
export function QcDashboard({ session }: { session: QcSession }) {
  ...
  const cards: { k: string; v: string }[] = [
    { k: "样本数", v: String(num(s, "sample_size", "total_samples") ?? "—") },
    { k: "细胞系数", v: cellsCount != null ? String(cellsCount) : "—" },
    { k: "序列长度", v: String(num(s, "length_consistency", "recommended_length_L") ?? "—") },
    { k: "含模糊碱基样本", v: String(num(s, "ambiguous_bases", "n_affected_sequences") ?? 0) },
    { k: "异常值", v: String(num(s, "outliers", "n_outliers") ?? 0) },
    { k: "总体", v: o.label },
  ];
  return (
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-6">
        {cards.map((c) => (
          <div key={c.k} className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
            ...
            <p className={`mt-0.5 truncate text-sm font-semibold ${c.k === "总体" ? o.cls : "text-slate-700"}`}>{c.v}</p>
          </div>
        ))}
      </div>
      {figures.length > 0 && (
