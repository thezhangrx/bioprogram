...
const stageLabel = (s: string) => STATUS_LABEL[s] ?? s;
  ...
  return (
    <div className="mx-auto flex h-full max-w-4xl flex-col items-center justify-center gap-8 px-6 py-10">
      <div className="text-center">
        <h1 className="text-2xl font-semibold tracking-tight text-slate-900">CRISPR 科学工作台</h1>
      ...
      <div className="grid w-full grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <button onClick={props.onQc}
          ...
          <p className="text-base font-semibold text-slate-800">数据质量体检</p>
          <p className="mt-1 text-sm text-slate-500">独立数据体检：只读检测结构与质量，无需进入训练</p>
        </button>
      </div>

      <div className="w-full">
        <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-400">项目列表</p>
        {err && <p className="mb-2 text-sm text-red-600">{err}</p>}
        {projects === null && <p className="text-sm text-slate-400">加载中…</p>}
        {projects !== null && projects.length === 0 && <p className="text-sm text-slate-400">暂无项目 — 点击上方创建，或从数据质量体检结果生成</p>}
...
function StageSummary({ stages }: { stages: Record<string, string> }) {
  ...
  return (
    <span className="flex items-center gap-1">
      {order.map((s) => (
        <span key={s} title={`${s}: ${stageLabel(stages[s] ?? "pending")}`} className={`size-2 rounded-full ${color[stages[s] ?? "pending"] ?? "bg-slate-200"}`} />
      ))}
    </span>
  );
}
