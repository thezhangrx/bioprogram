...
set -euo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"   # app/scripts/ -> 项目根
PORT_BACK="${PORT_BACK:-8765}"
PORT_FRONT="${PORT_FRONT:-5173}"
WS_ROOT="${CRISPR_WORKSPACE_ROOT:-$ROOT/workspace}"
WS_LOG_DIR="$ROOT/results/logs/app/workspace"
mkdir -p "$WS_LOG_DIR"

# 1) 后端
PYTHONPATH="$ROOT/app/backend" python3 -m crispr_workspace.server \
...
if [ ! -d "$ROOT/app/frontend/node_modules" ]; then
  ...
  (cd "$ROOT/app/frontend" && npm install --no-audit --no-fund)
fi
(cd "$ROOT/app/frontend" && npm run dev -- --port "$PORT_FRONT") > "$WS_LOG_DIR/frontend.log" 2>&1 &
FRONT_PID=$!
