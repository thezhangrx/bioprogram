    ...
    <div className="flex h-full">
      <aside className="w-60 shrink-0 border-r border-slate-200 bg-white">
        ...
        <nav className="p-2 text-sm">
          <p className="px-2 py-1 text-[11px] text-slate-400">Cell 01 · Name</p>
          {STAGE_ORDER.map((k) => k === "data" ? (
            <p key={k} className="px-2 py-1.5 text-[11px] text-slate-400">Cell 02 · Data Input</p>
          ) : (
          ...
          <p className="px-2 py-1 text-[11px] text-slate-400">Cell 08 · Project Files</p>
        </nav>
      ...
      <main className="flex-1 space-y-4 overflow-y-auto p-6">
        ...
        {/* Cell 01 · Name（项目名 + 输出目录弹窗） */}
        <NotebookCell index={1} title="Name" status={cfg.output_dir ? "completed" : "pending"}
        ...
        {/* Cell 02 · Data Input */}
        <NotebookCell index={2} title="Data Input"
        ...
        <NotebookCell index={3} title={TITLES.qc} status={m.stages.qc}
          hint={qcSession ? `session ${qcSession}` : "run QC on the imported dataset"}
          actions={
        ...
        {/* Cell 08 · Project Files */}
        <NotebookCell index={8} title="Project Files" status="completed" hint={filePath}>
