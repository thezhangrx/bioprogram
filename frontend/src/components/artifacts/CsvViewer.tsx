...
export function CsvViewer({ path, title }: { path: string; title?: string }) {
  ...
  if (err) return <p className="text-sm text-red-600">Unable to render artifact — {err}</p>;

  return (
    <div className="space-y-2">
      <div className="flex flex-wrap items-center gap-2 text-xs">
        <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="🔍 Search (当前页)…"
        ...
        <details className="relative">
          <summary className="cursor-pointer rounded-md border border-slate-200 px-2 py-1 hover:bg-slate-50">Columns</summary>
