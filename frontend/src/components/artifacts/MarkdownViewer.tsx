...
export function MarkdownViewer({ path }: { path: string }) {
  ...
  if (!state) return <p className="text-xs text-slate-400">Loading artifact…</p>;
  if (state.err) return <p className="text-sm text-red-600">Unable to render artifact — {state.err}</p>;
