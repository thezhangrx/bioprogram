    ...
    try {
      ...
      const poll = async () => {
        ...
        if (st.status === "completed") {
          setRun({ output: out, running: false, report: `${out}/summary/00_overview.md` });
          onCompleted?.(out);
        } else if (st.status === "failed") {
          setRun({ output: out, running: false, err: "analysis failed" });
        } else {
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap items-center gap-2 text-xs">
        ...
        <button onClick={() => void load()} className="rounded-md border border-slate-200 px-2 py-1 hover:bg-slate-50">Refresh</button>
            ...
            {tasks.filter((t) => (t.category || "core") === key).map((t) => {
              ...
              return (
                <label key={t.task_id} className={`flex items-center gap-2 rounded-md px-2 py-1 ${dis ? "opacity-60" : "hover:bg-slate-50"}`} title={dis ? t.reason : t.display}>
                  ...
                  <span>{t.display}</span>
                  {dis && <span className="text-[10px] text-rose-500">unavailable</span>}
                </label>
              );
            })}
            {tasks.filter((t) => (t.category || "core") === key).length === 0 && <p className="text-slate-400">(empty)</p>}
          </div>
        </fieldset>
      ))}
      ...
      <div className="flex items-center gap-2">
        <button onClick={() => void launch()} disabled={run?.running || !selected.size}
          ...
          {run?.running ? "Running…" : "Run Analysis"}
        </button>
        <span className="text-xs text-slate-400">{selected.size} selected · 状态逐 task 显示于 analysis_status.json</span>
      </div>
