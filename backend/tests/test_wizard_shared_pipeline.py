...
背景：桌面向导（``Input/backend_runner.py``）与网页工作台（``backend/crispr_workspace``）
...
wizard = importlib.import_module("Input.backend_runner")
from pipeline import build_command  # noqa: E402
...
class TestWizardUsesSharedSteps(unittest.TestCase):
    def setUp(self):
        ...
        self.dirs = dict(root_path=self.root,
                         proceeded_data_dir=ROOT / "data" / "proceeded_data")
    ...
    def test_training_options_forwarded(self):
        ...
        self.assertIn("data_digging.py", " ".join(cmd))
