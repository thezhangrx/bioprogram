...
- 可选任务列表来自 Analyse Registry (analyse.registry), 不在页面硬编码;
- availability 在提交时由引擎严格判定; 此服务提供基于产物的浅层提示 (GUI 只读);
- Run = 子进程调 analyse.pipeline (现有引擎), 状态从 analysis_status.json 读取。
"""
...
def registry_tasks() -> List[Dict]:
    """来自 analyse.registry 的任务目录 (找不到则回退到 engine 默认任务表)。"""
    try:
        from analyse.registry import registered_tasks
...
def run(batch_dir: str, output_dir: str, selected_task_ids: Optional[List[str]],
    ...
    """启动 analyse.pipeline (子进程); 状态落盘 analysis_status.json。"""
    out = Path(output_dir)
    ...
    cmd = [sys.executable, "-m", "analyse.pipeline",
...
def list_outputs(output_dir: str) -> Dict:
    ...
    entries = []
    for sub, kind in (("summary", "md"), ("tables", "csv"), ("figures", "png")):
        d = root / sub
        if not d.exists():
            continue
        for f in sorted(d.glob(f"*.{kind}")):
            entries.append({"name": f.name, "path": str(f), "kind": kind})
