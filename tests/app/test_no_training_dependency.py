...
仅以"路径字符串"或 analysis.registry 任务目录读取 (registry 只列任务, 不算科学计算)。
禁止模式: from core..., import core..., from train, import train, from predict,
...
PKG = Path(__file__).resolve().parents[2] / "app" / "backend" / "crispr_workspace"

FORBIDDEN = [
    re.compile(r"^\s*(from|import)\s+core\b"),        # 训练科学层不得被后端直接依赖
    re.compile(r"^\s*(from|import)\s+train\b"),
    re.compile(r"^\s*(from|import)\s+predict\b"),
    re.compile(r"^\s*(from|import)\s+data_digging\b"),
    re.compile(r"^\s*import\s+analysis\b"),          # 顶层整包 import 禁止
]
