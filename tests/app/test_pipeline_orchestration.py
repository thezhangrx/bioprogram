...
覆盖：
1. ``workflows.orchestrator.steps`` 注册表：命令构造、依赖拓扑、重复 id 检查、wizard 布局兼容；
...
from workflows.orchestrator import PipelineContext, STEPS, build_command, resolve_order  # noqa: E402
...
class TestStepRegistry(unittest.TestCase):
    ...
    def test_output_dir_derives_results_models_logs(self):
        """output_dir 之下固定派生 results/batches、models/weights、results/logs（统一约定）。"""
        ctx = PipelineContext(repo_root=ROOT, output_dir="/tmp/proj_out")
        self.assertEqual(ctx.results_path, Path("/tmp/proj_out/results/batches"))
        self.assertEqual(ctx.models_path, Path("/tmp/proj_out/models/weights"))
        self.assertEqual(ctx.logs_path, Path("/tmp/proj_out/results/logs"))
        cmd = build_command("collect_results", ctx)
        self.assertIn("--batch-dir", cmd)
        self.assertIn("/tmp/proj_out/results/batches", cmd)
...
class TestPipelineService(unittest.TestCase):
    def test_steps_payload_shape(self):
        # 交付结果位于 <repo>/results/batches/batch_20260909_full：
    ...
    def test_submit_command_streams_stdout_to_log(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            cmd = [sys.executable, "-c",
                   "print('[Step 1/3] hello from workflows.orchestrator'); print('done')"]
            ...
            self.assertIn("hello from workflows.orchestrator", text)
            self.assertIn("done", text)


class TestFilesService(unittest.TestCase):
    def test_list_and_preview(self):
        listing = files.list_dir("results/batches")
        self.assertTrue(any(e["name"] == "batch_20260909_full" for e in listing["entries"]))
        preview = files.read_file("results/batches/batch_20260909_full/summary/赛道二_results.csv")
