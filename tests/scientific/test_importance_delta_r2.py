...
class TestDirectionAndAlignment(unittest.TestCase):
    def test_delta_r2_direction(self):                       # 1
        ...
        cfg = AnalysisConfig()
        self.assertGreater(cfg.evidence.min_absolute_delta_r2, 0)
...
class TestEvidenceReuseAndExit(unittest.TestCase):
    ...
    def test_no_training_code_touched(self):                 # 16
        ...
        src = Path(__file__).resolve().parents[2] / "analysis" / "visualization" / "importance_delta.py"
        text = src.read_text(encoding="utf-8")
        for forbidden in ("import train", "from core.", "src.cnn", "src.xgboost",
                          "torch", "slurm", "sbatch"):
...
class TestRealBatchImportanceDelta(unittest.TestCase):
    def test_real_artifacts_if_present(self):
        tables = BATCH / "summary" / "tables" / "importance_vs_delta_r2.csv"
        module = Path(__file__).resolve().parents[2] / "analysis" / "visualization" / "importance_delta.py"
