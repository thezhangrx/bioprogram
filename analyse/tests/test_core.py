"""analyse.tests — 核心科学函数测试 (unittest, 无 pytest 依赖)。
...
from analyse.data.validation import metric_consistency_flags, validate_metric_consistency
from analyse.evidence.integration import (classify_cellline_consistency,
                                          classify_evidence_tier,
                                          direction_concordance)
from analyse.evidence.rules import (classify_attribution, classify_statistical_by_fdr)
from analyse.plans import AnalysisPlan, Capabilities, validate_analysis_plan
from analyse.schemas import EvidenceStrength, EvidenceTier
from analyse.stats.bootstrap import bootstrap_ci, bootstrap_difference_ci
from analyse.stats.effect_size import compute_paired_increment
...
class TestEvidenceRules(unittest.TestCase):
    def test_fdr_labels(self):
        cfg = AnalysisConfig()
        self.assertEqual(classify_statistical_by_fdr(0.0005, cfg), EvidenceStrength.STRONG_STATISTICAL)
        self.assertEqual(classify_statistical_by_fdr(0.005, cfg), EvidenceStrength.MODERATE_STATISTICAL)
        self.assertEqual(classify_statistical_by_fdr(0.03, cfg), EvidenceStrength.WEAK_STATISTICAL)
        self.assertIsNone(classify_statistical_by_fdr(0.2, cfg))

    def test_snr_never_called_statistical(self):
        # 即使 SNR 很高, 标签必须是 attribution, 不是 statistical significance
        cfg = AnalysisConfig()
        label = classify_attribution(9.0, effect_magnitude=0.05, config=cfg)
        self.assertEqual(label, EvidenceStrength.STRONG_ATTRIBUTION)
        self.assertNotIn("statistical", label.value.lower().replace("attribution", "attribution"))

    def test_min_effect_size_gate(self):
        # 效应极小但方差更小 -> 高 SNR 也不得称 Strong attribution
        cfg = AnalysisConfig()
        label = classify_attribution(9.0, effect_magnitude=1e-6, config=cfg)
