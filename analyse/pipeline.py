"""analyse.pipeline — 证据分析引擎编排入口 (只编排; 科学计算在各模块)。
...
from analyse import __version__ as ENGINE_VERSION
from analyse.config import AnalysisConfig
from analyse.data.loaders import coverage_summary, load_experiment_table
from analyse.data.validation import validate_metric_consistency
from analyse.plans import (AnalysisPlan, Capabilities, ExecutionPlan, ExecutionTask,
                           default_plan, validate_analysis_plan)
from analyse.reports.markdown_report import (build_anomaly_md, build_data_quality_md,
                                             build_overview_md)
...
class PipelineArtifacts:
    ...
    def create(cls, output: str | Path) -> "PipelineArtifacts":
        out = Path(output)
        for sub in ("tables", "summary", "figures"):
            (out / sub).mkdir(parents=True, exist_ok=True)
        return cls(out, out / "tables", out / "summary", out / "figures")
...
def _has_paired_predictions(batch_dir: Optional[str | Path], table: pd.DataFrame,
    ...
    if batch_dir is None or table is None or table.empty or "run_name" not in table.columns:
        return False
    from analyse.stats.tasks import predictions_path
...
def run_analysis(
    ...
    from analyse.prediction import loco_performance, performance_by_model_split
    from analyse.reports.markdown_report import (build_environment_md, build_prediction_md)

    executed: Dict[str, str] = {}
    completed_at = datetime.now(timezone.utc).isoformat()

    task_artifacts = {
        "qc": "summary/01_data_quality.md",
        ...
        "hypothesis_generation": "summary/07_biological_hypotheses.md",
    }
    ...
    if _should_run("bootstrap"):
        try:
            from analyse.stats.tasks import bootstrap_cellline_effects, bootstrap_environment_edges, bootstrap_main_effects, edge_ci_table
    ...
    if _should_run("hypothesis_testing"):
        try:
            from analyse.stats.tasks import (apply_fdr, permutation_environment_edges,
    ...
    if _should_run("environment_factorial_dag"):
        try:
            from analyse.environment.factorial_dag import write_factorial_dag_artifacts
            from analyse.visualization.factorial_dag import (
            ...
            dag_paths = write_factorial_dag_artifacts(
                batch_dir, artifacts.tables_dir, artifacts.summary_dir,
    ...
    if _should_run("sequence_attribution"):
        try:
            from analyse.attribution.extractors import extract_attribution_table
            from analyse.attribution.summary import build_motif_summary_md
            ...
            (artifacts.summary_dir / "04_sequence_motifs.md").write_text(
    ...
    if state.get("motif_discovery") is not None:
        ...
        if not plan.sequence.motif_discovery:
            ...
            try:
                from analyse.sequence.motif.pipeline import run_and_write
                motif_result = run_and_write(
                    attribution_table if attribution_table is not None else pd.DataFrame(),
                    batch_dir, artifacts.tables_dir, artifacts.summary_dir, artifacts.figures_dir,
                    config=config, discovery=True,
                    enrichment=bool(plan.sequence.motif_enrichment))
                from analyse.sequence.motif.pipeline import motif_evidence_rows
    ...
    if _should_run("cellline_heterogeneity"):
        if main_df is None:
            ...
            try:
                from analyse.cellline.consistency import (consistency_counts,
                                                          summarize_environment_by_cellline)
                from analyse.reports.markdown_report import build_cellline_md
                from analyse.stats.tasks import cellline_ci_lookup
                ...
                (artifacts.summary_dir / "05_cellline_heterogeneity.md").write_text(
    ...
    if _should_run("evidence_integration"):
        if main_df is None:
            ...
            try:
                ...
                from analyse.reports.markdown_report import build_evidence_md
                ...
                (artifacts.summary_dir / "06_evidence_integration.md").write_text(
    ...
    if _should_run("hypothesis_generation"):
        if evidence_matrix is None or evidence_matrix.empty:
            ...
            try:
                from analyse.reports.markdown_report import build_hypotheses_md
                (artifacts.summary_dir / "07_biological_hypotheses.md").write_text(
    ...
    (artifacts.summary_dir / "00_overview.md").write_text(overview_md, encoding="utf-8")
    ...
    (artifacts.summary_dir / "01_data_quality.md").write_text(
        build_data_quality_md(coverage, table, anomalies), encoding="utf-8")
    (artifacts.summary_dir / "08_anomaly_report.md").write_text(
        build_anomaly_md(anomalies, matrix_excluded), encoding="utf-8")

    if prediction_ok:
        (artifacts.summary_dir / "02_prediction_generalization.md").write_text(
            build_prediction_md(prediction_df, loco_df), encoding="utf-8")
    if cond_summary is not None and main_df is not None:
        (artifacts.summary_dir / "03_environment_effects.md").write_text(
    ...
    if dag_md_text:
        from analyse.environment.factorial_dag import append_dag_section
        append_dag_section(artifacts.summary_dir, dag_md_text)
    ...
    if executed.get("visualization") is None:
        ...
        try:
            from analyse.visualization.importance_delta import run_importance_delta_analysis
        ...
        # 6c) 常规图组
        try:
            from analyse.visualization import render_all
            render_all(
...
def main() -> None:
    ap = argparse.ArgumentParser(description="CRISPR evidence analysis engine (analyse.pipeline)")
    ap.add_argument("--batch-dir", type=str, required=True, help="训练结果批次目录")
    ap.add_argument("--output", type=str, default="", help="输出目录 (默认 <batch>/analyse_out)")
    ap.add_argument("--analysis-plan", type=str, default="", help="可选 AnalysisPlan json")
    args = ap.parse_args()

    batch = Path(args.batch_dir)
    output = Path(args.output) if args.output else (batch / "analyse_out")
