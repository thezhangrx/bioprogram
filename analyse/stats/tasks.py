"""analyse.stats.tasks — 把已实现的统计工具接到真实 effect 上 (bootstrap / permutation / FDR / ANOVA)。
...
from analyse.environment.incremental_effect import (ALL_ENVIRONMENTS, combo_name,
                                                    parse_environment_set)
from analyse.stats.bootstrap import bootstrap_paired_metrics_ci_fast
from analyse.stats.hypothesis_tests import (permutation_test_for_effect,
...
PERMUTATION_COLUMNS = ["split_type", "cell_line", "model", "random_seed", "test_type",
                       ...
                       "p_value", "n_permutations", "seed", "n_values", "family_key",
                       "status", "reason"]
...
def bootstrap_main_effects(main_df: pd.DataFrame,
    ...
    """factor 级 main effect 的跨模型 bootstrap CI (用于 Evidence Tier 的 CI 判定)。

    注意: 单位是"模型"的 main effect 估计 (n=可用模型数), 不是样本级 CI —— 输出中
    `estimate_basis` 明确记录, 避免被误读为样本级不确定性。
    """
    from analyse.stats.bootstrap import bootstrap_ci

    cfg = config or AnalysisConfig()
    if main_df is None or main_df.empty:
        return pd.DataFrame(columns=["feature", "estimate", "ci_low", "ci_high", "excludes_zero",
                                     "n_bootstrap", "seed", "n_models", "estimate_basis", "status"])
    rows: List[Dict] = []
    thr = float(cfg.consensus.unstable_effect_threshold)
...
def permutation_main_effects(cond_summary: pd.DataFrame,
    ...
    for keys, sub in cond_summary.groupby(["split_type", "cell_line", "model",
        ...
        vals = pd.to_numeric(sub["delta_r2_mean"], errors="coerce").dropna().tolist()
        ...
        if len(vals) < 3:
            rows.append({**ident, "observed_effect": float(np.mean(vals)) if vals else None,
                         ...
                         "n_values": len(vals), "status": "unavailable",
        ...
        rows.append({**ident, "observed_effect": res.observed, "p_value": res.p_value,
                     "n_permutations": res.n_permutations, "seed": cfg.bootstrap_seed,
                     "n_values": len(vals), "status": "ok", "reason": ""})
    return pd.DataFrame(rows, columns=PERMUTATION_COLUMNS)
