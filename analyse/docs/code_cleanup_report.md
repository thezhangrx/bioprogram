...
> 审计方法: AST 扫描 analyse/ 全部 .py (47 个文件, ~7.9k 行; 引擎新代码 ~3.3k 行),
...
|---|---|---|---|---|
| analyse/collect_results.py | 441 | 训练后指标汇总 (metrics_tables) | Input/backend_runner.py Step3 | 引擎 loaders 只读其结果文件, 不 import |
| analyse/anomaly_treatment.py | 489 | 两级异常检测 + anomaly_report.md (训练报告) | Input/backend_runner.py Step4 | 引擎 validation/integration 独立实现同语义; 08_anomaly_report 引用其"数据级报告"说明 |
| analyse/importance_extraction.py | 907 | 生信稳健性 md + key_regulatory_biomarkers.csv | backend_runner Step5; scripts/make_notebook.py | 引擎 extractors 的产物 (feature_importance/) 是输入 |
| analyse/visualization.py | 959 | V6 显著性热图/环境树图 (训练报告) | backend_runner Step6 (内部函数惰性 import) | **同名遮蔽**: 引擎包 analyse/visualization/ 遮蔽该 .py |
| analyse/data_QC.py | 1768 | 向导 Step2 无头质控 (训练侧, 用户绑定任务) | GUI/向导 (独立部署) | 不冲突; 引擎 QC 是批次级校验 |
...
旧 5 文件移至 `tools/legacy_analyse/` 归档, 引擎包移除桥接。
...
1. 删除引擎包内重复/错误位置文件: `src/xai_importance.py` 恢复训练端唯一位置 (曾误移
   analyse/), 引擎侧副本删除 → 训练导出不再 ModuleNotFoundError。
...
6. 遗留可清理项: analyse/README "重构进行中" 头注已随本文档周期移除 (见 §5 提交清单);
   __pycache__ 不入库。

## 5. 建议后续清理 (不阻塞当前交付)

- [ ] 训练端 backend_runner Step6 切换 render_all 之后: 归档 analyse/{visualization.py,
  collect_results.py, importance_extraction.py, anomaly_treatment.py} → tools/legacy_analyse/
