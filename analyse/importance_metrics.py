"""analyse.importance_metrics — Importance–ΔR² 二维图的“Y 轴指标注册表 + 证据筛选”规则。
...
设计原则 (与研究规范一致):
  ...
  * Y 轴只使用“有方向或至少有明确归因幅度定义”的主指标 (primary), 并按模型选择:
        ...
        mlp         : MLP_IG = mean(|IG|)       (IG_SNR 仅稳健性筛选; SmoothGrad 仅参考)
...
PRIMARY_METRICS: Dict[str, PrimaryMetricSpec] = {
    ...
    "mlp": PrimaryMetricSpec(
        ...
        note="Y=mean(|IG|); SmoothGrad 仅参考; IG_SNR 仅稳健性筛选"),
...
NOT_USED_AS_Y: List[Tuple[str, str, str]] = [
    ...
    ("SmoothGrad", "mlp", "平滑梯度参考量, 与 IG 语义重叠, 仅作参考"),
