# Submit/analyse/importance_extraction.py
"""
...
XAI_ALIAS = {
    ...
    "mlp": {
        ...
        "smoothgrad_mean": "SmoothGrad", "smoothgrad": "SmoothGrad", "sg_mean": "SmoothGrad",
        "ig_snr": "IG_SNR",
    },
    "cnn": {
...
def extract_mlp_importance(target_batch_dir: str, feature_importance_dir: str):
    """MLP —— XAI 白名单 (MLP_IG/SmoothGrad/IG_SNR), 无 p/FDR。"""
    records: List[Dict] = []
    csv_candidates = glob.glob(os.path.join(target_batch_dir, "**", "*.csv"), recursive=True)
    ...
    ok = _write_importance_md(
        ...
        "MLP Integrated Gradients & SmoothGrad Robustness (Whitelist)",
        XAI_WHITELIST["mlp"],
    )
    if ok:
