"""analyse.sequence.motif.pipeline — motif discovery 编排 (tables → figures → markdown)。
...
from analyse.sequence.motif.iupac import (IUPAC_TO_BASES, information_content,
...
def motif_evidence_rows(motifs: List[Dict], enrichment_rows: List[Dict]) -> List[Dict]:
    ...
    """
    from analyse.evidence.integration import classify_evidence_tier
    from analyse.schemas import EvidenceTier
    ...
    for m in motifs:
        ...
        strong = bool(statistical or (m.get("mean_effect") is not None and
                                      ...
                                      cfg.consensus.environment_strong_effect
