"""analyse.environment.factorial_dag — Environment Factorial DAG (2^4 lattice) 的数据层。
...
数据流 (禁止 visualization 自己扫描实验目录):
    existing results -> analyse.data.loaders (normalization)
        -> tables/environment_nodes.csv + tables/environment_edges.csv
        -> analyse.visualization.factorial_dag (纯 DataFrame 渲染)
"""
...
def merge_edge_ci(edges: pd.DataFrame, ci_table: Optional[pd.DataFrame]) -> pd.DataFrame:
    """合入真实的 bootstrap CI (analyse.stats.tasks.edge_ci_table 输出); 没有就保持 NaN。
...
def build_environment_interactions(table: pd.DataFrame) -> pd.DataFrame:
    ...
    from analyse.environment.incremental_effect import compute_pair_interactions
...
def generate_environment_factorial_dag(batch_dir: str | Path,
    ...
    summary_dir = Path(summary_dir) if summary_dir else (batch / "summary")
    ...
    from analyse.visualization.factorial_dag import (render_ablation_delta_r2,
