...
| 问题 | 证据 |
| :--- | :--- |
| 两套互不感知的编排器 | 向导 `Input/backend_runner.py` 自行拼 7 步 subprocess；网页 `backend/crispr_workspace/` 自行拼 CLI |
| 覆盖不一致 | 向导有 特征工程 / collect_results / anomaly_treatment / importance_extraction / 遗留可视化 / 交付物核对；网页**全都没有**（只有 QC、训练、analyse.pipeline） |
| 规则重复 | `data_digging.py` 注释写明"与 `Input/backend_runner.build_active_environment_combinations` 同规则"——复制而非共用 |
...
```text
        ...
        ┌────────────────▼───┐   ┌──────▼─────────────────────────┐
        │ 桌面向导            │   │ 网页工作台                      │
        │ Input/backend_runner│   │ backend/crispr_workspace/       │
        ...
        ┌───────────────────────────────────────────────┐
        │  同一批底层脚本（科学计算零改动）               │
        │  src/feature_engineering.py · data_digging.py  │
        │  predict.py · analyse/collect_results.py       │
        │  analyse/anomaly_treatment.py                  │
        │  analyse/importance_extraction.py              │
        │  analyse/visualization.py · analyse.pipeline   │
        └───────────────────────────────────────────────┘
```
...
| :--- | :--- | :---: | :--- |
| `feature_engineering` | data | | `data/proceeded_data/feature_schema.json`（仓库相对） |
...
```bash
python -m pipeline list                       # 步骤清单
python -m pipeline check                      # 按真实产物核对每步状态
python -m pipeline command --step train_grid  # 预览命令
python -m pipeline order                      # 依赖拓扑顺序
```
...
前端入口：`bash scripts/run_workspace.sh` → http://127.0.0.1:5173
...
* 验证：`backend/tests/test_pipeline_orchestration.py::TestRunLogStream`（快速子进程必须变为
...
| 项目 | 结果 |
| :--- | :--- |
| 共享层一致性（向导 vs 注册表） | `backend/tests/test_wizard_shared_pipeline.py`（6 项） |
| 步骤/文件服务 | `backend/tests/test_pipeline_orchestration.py`（13 项） |
...
```bash
# 1) 网页工作台（前端 → 后端 → 共享层）
bash scripts/run_workspace.sh         # 后端 :8765 + 前端 :5173
...
python -m pipeline command --step importance_extraction
```
...
| 批次定位方式 | 用户在 Cell 01「Name」用**目录弹窗**选择绝对输出目录 | `python -m pipeline command --step X --output-dir <abs> [--batch-name <name>]` |
...
调试用法示例（复核仓库内既有交付结果 `results/batch_20260909_full`）：

```bash
python -m pipeline check    --output-dir "$PWD" --batch-name batch_20260909_full
python -m pipeline command  --step collect_results --output-dir "$PWD" --batch-name batch_20260909_full
#   -> analyse/collect_results.py --batch-dir /.../Submit/results/batch_20260909_full
```
...
Cell 05 Training -> data_digging.py --results-dir /home/.../output/results --model-dir .../models --logs-dir .../logs
...
在测试级联删除时，指向 `results/batch_20260909_full` 的输出目录被整目录删除，
导致 1344 个实验目录与 `analyse_out` 丢失。已从当日 19:32 的打包备份
（`/tmp/test_upload.tar.gz`，含 10 301 个 `results/` 文件）完整恢复，
并新增回归测试 `test_delete_protects_nested_repo_dirs` 锁定该规则。

**恢复核验**：实验目录 1 344 个；`analyse_out` 表格 27 / 报告 15 / 图 293；
