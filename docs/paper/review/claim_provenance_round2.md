...
生成方式：全部数值由脚本从 `results/batches/batch_20260909_full/summary/` 与 `docs/paper_analysis/` 重算，未手工填写。
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| 1 | 4 个细胞系共 16 749 条合法 23 nt 序列 | §3.1 | `data/processed/*_metadata.csv` | 计数 | L1 | 否 |
...
| 候选工具与设计创新 | 20 | Table 6 给出 4 个候选（C1 第 18 位、C2 GAGG、C3 GGGG、CTGG 参考），含多模型支持、效应、稳健性、最小扰动、优先级；`workflows/prediction/predict.py --generate-candidates` 提供候选排序 | 中 | 候选为"已有序列排序 + 假设"，非 de novo 生成；尚无实验闭环 | ✅ §2.10/§3.8/§7 | 湿实验或独立数据集验证；GenAI 序列生成模块 |
...
| Table 1 | 数据集组成 | `data/processed/*_metadata.csv` |
...
| Table S1/S2 | 阈值清单 / 跨模型位置一致性 | `analysis/config.py`、`docs/paper_analysis/cross_model_position_consistency.csv` |
