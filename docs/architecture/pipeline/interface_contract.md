...
```bash
python -m analysis.pipeline --batch-dir <batch_dir> --output <out_dir> [--analysis-plan plan.json]
```

- `--batch-dir`: 训练结果批次目录 (含 summary/metrics_tables/all_experiments.csv,
  可带 legacy 实验子目录)。
- `--output`: 默认 `<batch>/summary`。输出目录结构固定:
...
- 引擎绝不 import 训练模块 (src/, workflows/prediction/predict.py); legacy analysis/*.py 由训练端调用, 引擎不依赖。
