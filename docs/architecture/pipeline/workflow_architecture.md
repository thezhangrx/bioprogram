...
> 证据分析引擎只读消费训练产物; 训练系统 (src/, workflows/prediction/predict.py, app/desktop/) 不被分析需求修改。
...
| 层 | 模块 | 职责 |
|---|---|---|
| 配置 | `analysis/config.py` | 阈值集中: SNR≥2.5/min_effect_size/FDR 档位/consensus/QC/bootstrap-seed |
| Schema | `analysis/schemas.py` | ExperimentRecord/AttributionRecord/StatisticalEvidence/… + Evidence 标签 |
| 计划 | `analysis/plans.py` | AnalysisPlan → Validator (selected/available/status) → ExecutionPlan/status json |
| 编排 | `analysis/pipeline.py` | `run_analysis()`; 任务状态机 pending→completed/failed/unavailable/skipped |
| 注册 | `analysis/registry.py` | 任务目录 (新增分析=注册, GUI 零改动) |
| 数据 | `analysis/data/` | 统一实验表加载 (metrics_tables 优先, legacy 目录扫描适配) + 一致性校验 |
| 统计 | `analysis/stats/` | Δ+paired-baseline / BH-FDR / bootstrap CI / permutation (runner 待挂载; 包名避开标准库 statistics) |
| 归因 | `analysis/attribution/` | whitelist 字段 → 统一 attribution 表 (7 方法 × 位置通道) |
| 环境效应 | `analysis/environment/` | 条件增量 / 主效应 (同 seed cohort 配对) |
| 细胞系 | `analysis/cellline/` | context 一致性 / 跨细胞系汇总 |
| 证据 | `analysis/evidence/` | Evidence Strength 标签 / matrix + Tier / 受控假设语言 |
| 报告 | `analysis/reports/` | summary/0X_*.md 生成 |
| 图形 | `analysis/visualization/` | render_all(统一表) → figures/ 分主题 PNG |
