...
- 科学计算 (QC/Training/Analyse/Statistics/XAI/Reports) 只经 **现有引擎**: 只读产物 或
  子进程调用 `workflows/training/data_digging.py / workflows/prediction/predict.py / analyse.pipeline / data_QC.py`。
...
└── crispr_workspace/          # 本地 Workflow API (零第三方依赖, stdlib http.server)
    ...
    └── server.py              /api/* 路由
app/backend/tests/                 (18 项: workspace/workflow/e2e/recovery)
app/frontend/                      React18+TS+Vite+Tailwind
...
app/scripts/run_workspace.sh / deploy/hpc/build_upload.sh
```
...
- 任务目录来自 `analysis/registry`; Selected/Available(含 reason)/Unavailable/Skipped/Completed 由引擎与启发式共同呈现, 最终由引擎运行时判定。
...
## 11. 运行
```bash
bash app/scripts/run_workspace.sh           # 后端 8765 + 前端 5173 (打开 http://127.0.0.1:5173)
# 后端独立: PYTHONPATH=backend python3 -m crispr_workspace.server --port 8765
# 打包上传: bash deploy/hpc/build_upload.sh /tmp/Submit_upload.tar.gz
```
...
* **共享步骤定义**：`pipeline/steps.py`（向导与网页共用）；服务层 `app/backend/crispr_workspace/pipeline.py`。
