...
1. **`all` 划分在网格中等价于 `single`（逐位相同）**：`data_digging.py` 只传 `--cell-line`，
   `train.py:663-664` 回填 `cell_lines=[cl]`，`split_all_cell_lines:214-217` 的退化保护改为细胞系内划分；
...
3. **当前 schema**：`[A,C,G,T,CTCF,Dnase,H3K4me3,RRBS]`、23×8=184；线性回归单独剔除 23 个 `_T` 列 → 161
   （`src/xai_importance.py` 白名单与 `linear_regression.select_non_t_reference_features`）。
4. **XAI 实况**：XGB G/W/C+TreeSHAP、MLP IG+SmoothGrad、CNN IG+ISM（全部 23×C 位点翻转）、
   Transformer 注意力+熵（熵是**全局标量广播**到所有位置）；**未实现**：permutation test、
   SmoothGrad_SNR、CNN_IG_SNR；Transformer IG 有函数但**从未被调用**。SNR 星级阈值只在
   `analyse/importance_extraction.py` 后处理里，`src/` 内没有。
5. **predict.py 与网格训练不是同一训练机制**：predict.py 直接 import 模型类并以
   AdamW/lr=1e-3/batch=256/epochs=15（默认）训练、10 折 `KFold`、**无早停**；网格路径经 `train.py`
   使用 Adam/MSE/batch=64/epochs=100/patience=20。predict.py 的断点续跑判据是 `ultimate/progress.json`
   + `preds_<kind>.npy`，**不扫描 results/batch**。
6. **实跑分析产物规模（1344 实验批次）**：`analyse_out` 9 篇 summary、10 张 tables、15 张 figures；
...
- 2. 数据输入
  ...
  - 2.2 原始 CSV → NPY：`Input/backend_runner.py`（向导路径）
  - 2.3 已落盘数据资产（只读校验）
- 3. 数据质量控制 (QC)
  - 3.1 `analyse/data_QC.py`（1768 行）
    - 3.1.1 模块级常量（默认阈值集中处）
    - 3.1.2 基础 IO / 统计工具函数
    - 3.1.3 `class DataQualityController`（`:371`）
    - 3.1.4 顶层入口与 CLI
  - 3.2 QC 产物 schema
  - 3.3 结果级 QC：`analyse/anomaly_treatment.py` + `analyse/data/validation.py`
    - 3.3.1 `analyse/anomaly_treatment.py`（489 行）
    - 3.3.2 `analyse/data/validation.py`（63 行）
  - 3.4 QC → 用户决策 边界
- 4. 数据映射与预处理
  - 4.1 序列编码
  - 4.2 表观/环境通道编码
  - 4.3 去重 / 缺失 / 掩码 / 标准化
- 5. 特征工程
  - 5.1 `src/feature_engineering.py` 函数级清单
    - 配置加载与校验
    - 编码
    - schema 与命名
    - 读入、清洗与变换
...
- **模型训练与 XAI（§7–§10）**
- 7. 模型训练
  - 7.1 训练总入口 train.py (CLI→流程)
    - 7.1.1 顶部常量
    - 7.1.2 CLI 参数表（`parse_args`, train.py:599-644）
    - 7.1.3 主流程（`run_one_experiment`, train.py:441-592）
    - 7.1.4 scaler 的 fit-on-train 证据（train.py 只传标志，不拟合）
    - 7.1.5 目录与 run_name 约定
  - 7.2 五模型并行/分发机制
    - 7.2.1 模型名 → 模块映射（唯一权威表）
    - 7.2.2 data_digging.py（Training Scope 网格）→ 子进程调用 train.py
    - 7.2.3 predict.py（Ultimate mixed 十折）**不调用 train.py**
    - 7.2.4 GUI/后端（backend/crispr_workspace/training.py）
  - 7.3 Linear Regression（src/linear_regression/linear_regression.py，852 行）
    - class `LinearRegressionModel` (linear_regression.py:56)
    - 模块级函数
    - 输入/输出/训练流程（`train`, linear_regression.py:673-816）
  - 7.4 XGBoost（src/xgboost/xgboost.py，694 行）
...
- 8. XAI / Feature Attribution
  ...
  - 8.2 各方法函数级说明
    ...
    - `compute_smoothgrad(model, X, device, n_samples=20, noise_level=0.15)` — mlp.py:245
  ...
  - 8.4 白名单列（`WHITELIST_COLUMNS`, src/xai_importance.py:29-36）
- 9. 训练输出与实验目录结构
- 10. 与 README/注释不一致之处
  - 本节自检
- **结果收集与 Analyse 分析引擎（§11–§14）**
- 11. 实验结果收集与汇总 (legacy scripts; each with 状态/职责/输入/输出/函数清单)
  - 11.1 analyse/collect_results.py (441 行) — 指标汇总表生成
    - 实验发现与解析规则
    - 排序 / 过滤 / ΔR²
    - 函数清单
    - CLI 与产出定位
  - 11.2 analyse/importance_extraction.py (908 行) — 特征重要性抽取与白名单报告
    - 常量与白名单（XAI 学术红线）
    - 函数清单
  - 11.3 analyse/visualization.py (959 行) — V6 显著性掩码热图 / 环境增量树 / 表观对比
    - 关键函数
  - 11.4 analyse/anomaly_treatment.py (489 行) — 两级异常检测报告
    - 函数清单
    - 函数与规则
- 12. Analyse 证据分析引擎
  - 12.1 总览与运行入口
  - 12.2 配置与阈值（`analyse/config.py`，全部默认值）
  - 12.3 schemas 与 Evidence 标签（`analyse/schemas.py`）
    - 标签 → 生产使用映射（grep 实证）
  - 12.4 plans / registry / capabilities
  - 12.5 pipeline 逐阶段
  - 12.6 data 层
    - `analyse/data/loaders.py`
    - `analyse/data/validation.py`
  - 12.7 statistics
  - 12.8 environment 增量与主效应（`analyse/environment/incremental_effect.py`）
  - 12.9 attribution 抽取
    - 列适配 `analyse/attribution/columns.py`
    - 行抽取 `analyse/attribution/extractors.py`
  - 12.10 cell-line 一致性
  - 12.11 evidence 整合
    - `analyse/evidence/rules.py`（标签规则，阈值全部来自 config）
    - `analyse/evidence/integration.py`（证据矩阵）
    - `analyse/evidence/hypothesis.py`（受控语言）
  - 12.12 reports & visualization
    - `analyse/reports/markdown_report.py`（builder → 文件）
    - `analyse/visualization/`（新包，`render_all`）
  - 12.13 产物 schema（实跑 `results/batch_20260909_full/analyse_out/`）
    - 目录
    - summary/*.md（编号 = 分析阶段）
    - tables/*.csv（列名 = 实测首行）
  ...
  - 不稳定隔离与字段语义
  - 测试规模与死码审计
- 13. 测试体系（`analyse/tests/`，unittest，无 pytest 依赖）
- 14. 与文档/注释不一致或未实现项
  - 本节自检
- **运行器 / Wizard / Workflow / 前端 / Runtime（§15–§23）**
- 15. 实验运行器 (predict.py / data_digging.py: 职责边界, CLI 参数表, 调用链)
  - 15.1 职责边界 (由代码 docstring 与 main() 实证)
  - 15.2 predict.py CLI 参数表 (`parse_args`, predict.py:723-754)
  - 15.3 通道规划与目标特征 (Target Epigenetics)
  - 15.4 Ultimate 模型与网格
  - 15.5 RESUME 机制 (predict.py)
  - 15.6 输出 `赛道二_results.csv`
  - 15.7 data_digging.py CLI 参数表 (`parse_args`, data_digging.py:384-433)
  - 15.8 网格实验引擎 (data_digging.py)
- 16. Wizard / Workflow (main_wizard 7 步 form_data 字段表; backend_runner 步骤 1–7 调用链; 条件分支; 自动 vs 用户决策)
  ...
  - 16.4 校验 (自动) vs 用户决策
  - 16.5 backend_runner.execute_full_pipeline 步骤 1–7 (`Input/backend_runner.py:144-306`)
- 17. Local Workflow API (backend/crispr_workspace 逐文件函数级; 完整 API 表)
  - 17.1 config.py (48 行)
  - 17.2 store.py (41 行)
...
- 附录（自动生成索引）
  - 附录 A：程序索引（按目录）
  - 附录 B：函数清单（按程序）
    - data_digging.py  (518 lines, Active)
    - predict.py  (1008 lines, Active)
    - train.py  (734 lines, Active)
    - Input/backend_runner.py  (306 lines, Active)
    - Input/main_wizard.py  (536 lines, Active)
    - analyse/anomaly_treatment.py  (489 lines, Active)
    - analyse/collect_results.py  (442 lines, Active)
    - analyse/config.py  (65 lines, Active)
    - analyse/data_QC.py  (1768 lines, Active)
    - analyse/importance_extraction.py  (908 lines, Active)
    - analyse/pipeline.py  (345 lines, Active)
    - analyse/plans.py  (242 lines, Active)
    - analyse/prediction.py  (35 lines, Active)
    - analyse/registry.py  (77 lines, Active)
    - analyse/visualization.py  (959 lines, Active)
    - src/feature_engineering.py  (2029 lines, Active)
    - src/xai_importance.py  (135 lines, Active)
    - analyse/attribution/columns.py  (61 lines, Active)
    - analyse/attribution/extractors.py  (139 lines, Active)
    - analyse/attribution/summary.py  (50 lines, Active)
    - analyse/cellline/consistency.py  (60 lines, Active)
    - analyse/data/loaders.py  (173 lines, Active)
    - analyse/data/validation.py  (63 lines, Active)
    - analyse/environment/incremental_effect.py  (170 lines, Active)
    - analyse/evidence/hypothesis.py  (74 lines, Active)
    - analyse/evidence/integration.py  (194 lines, Active)
    - analyse/evidence/rules.py  (73 lines, Active)
    - analyse/reports/markdown_report.py  (180 lines, Active)
    - analyse/stats/bootstrap.py  (107 lines, Active)
    - analyse/stats/effect_size.py  (32 lines, Active)
    - analyse/stats/hypothesis_tests.py  (76 lines, Active)
    - analyse/stats/multiple_testing.py  (35 lines, Active)
    - analyse/tests/test_attribution.py  (78 lines, Test-only)
    - analyse/tests/test_core.py  (201 lines, Test-only)
    - analyse/tests/test_environment.py  (70 lines, Test-only)
    - analyse/tests/test_phase5.py  (63 lines, Test-only)
    - analyse/tests/test_phase6.py  (110 lines, Test-only)
    - analyse/visualization/__init__.py  (84 lines, Active)
    - analyse/visualization/attribution_plots.py  (38 lines, Active)
    - analyse/visualization/cellline_plots.py  (41 lines, Active)
    - analyse/visualization/core.py  (21 lines, Active)
    - analyse/visualization/environment_plots.py  (43 lines, Active)
    - analyse/visualization/evidence_plots.py  (36 lines, Active)
    - analyse/visualization/performance_plots.py  (35 lines, Active)
    - backend/crispr_workspace/analysis.py  (256 lines, Active)
    - backend/crispr_workspace/artifacts.py  (158 lines, Active)
    - backend/crispr_workspace/config.py  (48 lines, Active)
    - backend/crispr_workspace/fingerprint.py  (61 lines, Active)
    - backend/crispr_workspace/project.py  (136 lines, Active)
    - backend/crispr_workspace/qc_service.py  (141 lines, Active)
    - backend/crispr_workspace/server.py  (279 lines, Active)
    - backend/crispr_workspace/store.py  (41 lines, Active)
    - backend/crispr_workspace/training.py  (494 lines, Active)
    - backend/tests/test_e2e_smoke.py  (127 lines, Test-only)
    - backend/tests/test_no_training_dependency.py  (42 lines, Test-only)
    - backend/tests/test_project_recovery.py  (54 lines, Test-only)
    - backend/tests/test_workflow_interfaces.py  (131 lines, Test-only)
    - backend/tests/test_workspace.py  (157 lines, Test-only)
    - src/cnn/cnn.py  (873 lines, Active)
    - src/input_control/cell_environment_combination.py  (1908 lines, Active)
    - src/input_control/cell_line_division.py  (334 lines, Active)
    - src/linear_regression/linear_regression.py  (852 lines, Active)
    - src/mlp/mlp.py  (801 lines, Active)
    - src/transformer/transformer.py  (805 lines, Active)
    - src/xgboost/xgboost.py  (694 lines, Active)
...
- 前端：`frontend/src/` 32 个 `.ts/.tsx` 文件 + `package.json`/`vite.config.ts`。
- 脚本：`scripts/run_workspace.sh`、`scripts/build_upload.sh`；配置：`data/feature_config.json`、`data/proceeded_data/feature_schema.json`。
...
```text
 M README.md
MM analyse/collect_results.py
MD analyse/visualize_results.py          # 工作区已删除该文件
AM data/feature_config.json
AM data/proceeded_data/feature_schema.json
 D data/proceeded_data/*_161.* / *_23x7.*   # 旧 7 通道产物被删除
...
（例如 `train.py`、`data_digging.py`、`analyse/` 新引擎多数文件、`src/xai_importance.py`、`scripts/run_workspace.sh` 等，见 `git status --short -- . | grep '^??'`）。
因此：**当前代码库没有对应这些实现的提交版本**，本快照的复现基线应以工作区文件为准（而非 commit `9b2def2b`）。
> 另：`analyse/visualize_results.py` 在工作区已不存在（`ls` 实测缺失）；`analyse/visualization.py` 才是现存的旧单文件可视化脚本。
...
2. `README.md` 曾写 `visualize_variant_importance.py`，实际文件为 `analyse/visualization.py`（已修正）。
3. `analyse/visualization.py`（旧单文件）与 `analyse/visualization/`（新包）**同名冲突**：包优先；包内 `__init__.py` 用 PEP 562 `__getattr__` 惰性桥接旧模块的 `generate_all_visualizations`，`Input/backend_runner.py` Step6 因此仍可用。
4. `predict.py` 中 `train_ultimate_models()` 与 `generate_track2_results_ultimate()` 仍存在但 **`main()` 已不再调用**（被 `run_ultimate_with_resume()` 取代），属 Unused。
5. `analyse/registry.py` 登记的部分任务（motif discovery 等）在 `analyse/pipeline.py` 中状态为 `unavailable`（未实现，附 reason），属 Planned。
6. `stats/hypothesis_tests.anova_interface` 为占位（`available=False`），不是已实现 ANOVA。
7. `workspace/` 目录为运行期默认工作根（可能为空），不属于代码。
8. **接口不一致（实测）**：`analyse/registry.py` 的任务规格字段是 `name`（无 `display`），而
   `backend/crispr_workspace/analysis.py::registry_tasks()` 读取的是 `display` → 未命中时回退为 `task_id`，
   因此前端任务列表显示的是 task id 而非人类可读名称。
9. **前端分组不一致（实测）**：`frontend/src/components/AnalysisPanel.tsx` 按 `core/advanced` 两组过滤，
   而 registry/后端给出的 `category` 取值是 `qc/biology/statistics/evidence` → 该面板当前会把所有任务过滤为空
   （需在后续工程修复；本次仅记录）。
10. `analyse/registry.py` 中所有内置任务的 `runner=None`：**runner 未通过 registry 注册**，
    实际阶段实现直接写在 `analyse/pipeline.py` 的 `run_analysis()` 中（registry 目前仅作任务目录/UI 来源）。
...
## 1.1 总流程图（真实实现的三种主要入口）
```text
入口 A：7 步 GUI 向导 (Input/main_wizard.py → Input/backend_runner.py)
入口 B：Scientific Workspace (scripts/run_workspace.sh → backend/crispr_workspace/server.py + frontend/)
入口 C：各阶段独立 CLI (data_digging.py / predict.py / analyse.pipeline / data_QC.py / analyse/*.py)

入口 A 的执行顺序（execute_full_pipeline 实测顺序）:
[Step1] 特征工程 (run_feature_engineering_step / convert_raw_csv_to_npy)
   ↓
[Step2a] 网格挖掘 data_digging.py（Training Scope 环境组合 × 模型）→ 每实验子进程 train.py
   ↓
[Step2b] 条件分支：仅当 has_target（勾选目标基因组）→ predict.py（mixed 十折 CV + 目标预测）
   ↓
[Step3] analyse/collect_results.py（→ summary/metrics_tables/）
   ↓
[Step4] analyse/anomaly_treatment.py（→ summary/anomaly_report.md）
   ↓
[Step5] analyse/importance_extraction.py（→ summary/feature_importance/ + key_regulatory_biomarkers.csv）
   ↓
[Step6] analyse/visualization.py::generate_all_visualizations（经同名包兼容桥）→ summary/plots/
   ↓
...
## 1.2 分析引擎（analyse）的执行顺序
`python -m analyse.pipeline --batch-dir <batch> --output <out>`（`analyse/pipeline.py::run_analysis`）按注册任务逐段执行：
```text
...
| 条件 | 行为 | 位置 |
|---|---|---|
| GUI 勾选“候选设计/目标基因组” | 追加 `predict.py --target-input ... --target-epigenetics ...` | `Input/backend_runner.py` Step2b |
| 环境组合 = 4 个表观全部 | 归一化为单一 `all`（丢弃显式四元名，避免 17 组合→1428 实验） | `data_digging.py::_canonicalize_combinations` |
| 用户选择 models/cell-lines/split-types | 决定 `generate_experiments` 笛卡尔积 | `data_digging.py::generate_experiments` |
| `--in-process` | 复用解释器顺序执行（避免每实验 python 启动开销） | `data_digging.py::run_experiment_in_process` |
| `--workers>1` | 实验级并发（每个实验仍是相同 train.py 子进程） | `data_digging.py::build_worker_env/run_one_experiment` |
| `--threads-per-worker>0` | 并发时限制 OMP/MKL 线程（**会改变浮点求和顺序**，默认关闭） | 同上（代码注释明确标注风险） |
| predict 断点续跑 | `ultimate/progress.json` + `preds_<kind>.npy` 完成标记，存在则跳过该模型 | `predict.py::run_ultimate_with_resume/_kind_done` |
| data_digging 断点续跑 | 扫描 results/[batch]/ 实验目录 info 判定 completed（**不看 batch 中“有目录”即完成**，只有 info/metrics 齐全才算） | `data_digging.py::build_completed_lookup/classify_experiments` |
| 分析任务未选中 | status=skipped，不执行 | `analyse/plans.py::validate_analysis_plan` |
| XAI 计算开关 | GUI `xai_vars`（linear/xgb/mlp/cnn/transformer 各自开关）→ 传 train.py `--compute-shap` 等 | `Input/main_wizard.py` Step5 / `train.py` |

并行执行：网格实验按“一个 GPU 一个 `data_digging.py` 进程 + 环境切片”并行（见 `HPC_EXPERIMENT_PROTOCOL.md` §2）；
五类模型在同一进程内按顺序训练（每个实验一个模型），GUI/Workflow 层面可多卡并行。

## 1.4 数据流（形态变化，逐段标注负责程序）
```text
原始 CSV（每细胞系一个文件；列含 sgRNA / Chromosome / Start / End / 表观轨道 / efficacy）
  │  src/feature_engineering.py::load_source_csv（去重 remove_duplicate_rows）
  ▼
...
features_3d (N, 23, C)  +  features_2d (N, 23*C)  +  labels (N,)
  ...
  ▼
data/proceeded_data/ : {cell}_23x{C}.csv, {cell}_{23*C}.csv,
                       {cell}_features_23x{C}.npy, {cell}_features_{23*C}.npy,
                       {cell}_labels.npy, {cell}_metadata.csv, feature_schema.json
  │  train.py::prepare_split_data → src/input_control/cell_line_division.divide_data
  ▼
X_train_3d / X_valid_3d / X_test_3d （single=按细胞系内部随机划分；all=留一细胞系；mixed=跨细胞系混合+多种子）
  │  train.py::prepare_model_data → cell_environment_combination.prepare_train_valid_test
  ▼
...
每实验目录 results/<batch>/<run_name>/：
  ...
  │  analyse/collect_results.py::collect_batch
  ▼
summary/metrics_tables/all_experiments.csv（规范列；另出 single/all/mixed 汇总表与 baseline.csv）
  │  analyse/pipeline.py::load_experiment_table（统一表；ids 小写化、metrics 保大写）
  ▼
canonical experiment_table.csv
  │  environment / attribution / cellline / evidence / reports / visualization
  ▼
analyse_out/tables/*.csv + summary/00..08.md + figures/*.png
```
> 关键 shape 事实（来自 `data/proceeded_data/feature_schema.json` 与 FE 代码）：
...
> **只读声明**：本节编写过程未修改仓库内任何文件；仅以只读方式 `np.load(..., mmap_mode="r")` / `pd.read_csv(nrows=...)` 校验了 `data/proceeded_data/` 的真实 shape。
...
| :--- | :--- | :--- | :--- | :--- |
| ① 7 步 Tk 向导 | `Input/main_wizard.py` → `Input/backend_runner.py` | `_launch_pipeline` → `execute_full_pipeline` | **Active** | `Input/backend_runner.py:144` 被 `main_wizard` 的启动回调调用 |
| ② 特征工程 CLI | `src/feature_engineering.py` | `python src/feature_engineering.py --source-dir … --output-dir … --config …` | **Active（数据实际来源，但运行时不被调用）** | 见 §2.3 判定证据 |
| ③ Workspace HTTP 后端 | `backend/crispr_workspace/server.py` + `qc_service.py` | `POST /api/qc/sessions` | **Active** | `server.py:92-99`；QC 引擎以子进程调用 |

**关键事实：向导第 2 步不是 QC。** `Input/main_wizard.py:146` 的第 2 步标题是「选择已测量数据与待预测基因组」，全文件不含 `data_QC` / `qc` 字样（grep 无命中）。`analyse/data_QC.py:5` 声称「供 7 步交互式向导 (GUI/CLI) 第二步调用」——当前仓库不存在这条调用链，真实集成方是后端 `qc_service.QCSessionManager._launch`（`backend/crispr_workspace/qc_service.py:116-133`）。

### 2.1.2 原始 CSV 的真实列结构

真实表头（`data/source_data/*.csv`，4 个细胞系一致）：
...
- 目标列名固定为 `Normalized efficacy`（`src/feature_engineering.py:134` `TARGET_COLUMN`）。
- 元数据列固定 6 个（`src/feature_engineering.py:136-143` `METADATA_COLUMNS`）：`Cell line`（处理时由代码 `df.insert(0, "Cell line", cell_line)` 注入，`src/feature_engineering.py:1229-1233`）、`Chromosome`、`Start`、`End`、`Strand`、`sgRNA`。
- 4 条环境轨道为 23 字符的 `A`/`N` 串（不是数值）；`A` 表示该位点有注释、`N` 表示无注释。
- `data/todo_data.CSV` 是另一种形态（仅 `sgRNA,Efficacy` 两列，无坐标、无环境轨道），**不满足** `get_required_columns()` 的必需列要求（`src/feature_engineering.py:1030-1060`），无法直接进入本流水线；仓库内除文件本身外无任何代码引用它。
- 向导默认列名（`Input/main_wizard.py:30-38`）：`seq_col="sgRNA"`、`target_col="Normalized efficacy"`、`epi_cols_raw="CTCF, Dnase, H3K4me3, RRBS"`、`chrom_col="Chromosome"`、`start_col="Start"`、`end_col="End"`、`seq_len=23`。

### 2.1.3 列自动识别与编码自适应（QC 侧）

`analyse/data_QC.py` 不要求用户显式给列名，使用三级别名匹配：
...
**Status: Active**（`execute_full_pipeline` 第 1/7 步，`Input/backend_runner.py:177-182`）。
...
- `convert_raw_csv_to_npy(csv_path, cell_line, output_dir, form_data)`（`:40-91`）：**向导路径真正的编码器**，与 `src/feature_engineering.py` 是两套独立实现。
...
- `run_feature_engineering_step(form_data, raw_data_dir, output_data_dir, target_cell_lines)`（`:94-141`）：
  - 先把仓库基准 `data/proceeded_data/*.*` 中「目标目录尚不存在」的文件拷入输出目录（`:100-105`）；
...
- `execute_full_pipeline(form_data, root_output_dir)`（`:144-306`）：7 步编排（特征工程 → `data_digging.py` → 可选 `predict.py` → `collect_results.py` → `anomaly_treatment.py` → `importance_extraction.py` → `visualization.generate_all_visualizations`），每步以 `subprocess.run` 调用，返回码非 0 仅打印 `[Warning]` 不中断（`:206-207`、`:240-241`）。

## 2.3 已落盘数据资产（只读校验）

`data/proceeded_data/` 真实内容（`np.load(mmap_mode="r")` + 行数统计）：
...
`data/proceeded_data/feature_engineering_summary.csv` 实测内容：
...
**来源判定**：该 summary 文件的列名恰为 `process_one_csv` 的返回字典键（`src/feature_engineering.py:1739-1757`）且只由 `process_all_csv` 写出（`:1899-1911`）；`Input/backend_runner.py` 不生成该文件。因此仓库内 `proceeded_data/*.npy` 是 **`src/feature_engineering.py` CLI 的产物**，而运行时向导走的是 `backend_runner.convert_raw_csv_to_npy`。两者产物命名相同、语义有差异（见 §6.5）。
...
**Status: Active**——被 `backend/crispr_workspace/qc_service.py:24`（`ENGINE = Path("analyse")/"data_QC.py"`）以子进程调用（`--output-dir <session> --data <inputs[0]>`），前端 `frontend/src/components/QcDashboard.tsx` 只读渲染其 `qc_summary.json`。**全模块只读：无任何 `to_csv` 写回原始数据、无 `drop`/`replace`/`fillna` 落盘，唯一写出的表格是独立命名的 `outliers_detailed_report.csv`。**
...
**注意**：`analyse/config.py:41-47` 另有一份 `QCConfig`（`environment_missing_rate_limit=0.30`、`strict_complete_gate=70.0`、`ambiguous_detection_only=True`），但 `data_QC.py` 的 import 列表中**没有** `analyse.config`（`:27-47`），即这些阈值存在两处定义且未打通（见 §6.5）。
...
- `_demo_data()`（`:1666-1676`）/ `_synthesize_demo()`（`:1679-1714`）：优先用 `data/source_data`；否则在临时目录合成 3 个脏数据 CSV（含 `N` 碱基、长度异常、`H3K4me3` 缺失列）。
...
**Status: Active**——由 `Input/backend_runner.py:255-261` 在流程第 4/7 步以子进程调用（`--batch-dir <results>`）。
...
- `load_metrics_table(batch_dir) -> DataFrame`（`:92-117`）：优先 `<batch>/summary/metrics_tables/all_experiments.csv`，再 `<batch>/summary/all_experiments.csv`（`:93-96`）；都不可用时回退调用 `analyse.collect_results.collect_batch`（`:106-116`）。该 CSV 由 `analyse/collect_results.py:332` 写出。
...
**Status: Active**——由 `analyse/pipeline.py:21` import、`:84` 调用（`anomalies = validate_metric_consistency(table)`），结果写 `tables/metric_inconsistency.csv`（`pipeline.py:85`）；单测见 `analyse/tests/test_core.py:16,42-50,163`。
...
1. **QC 只检测、不修改数据**：`analyse/data_QC.py` 全文无写原始 CSV、无 `drop_duplicates`、无 `replace`、无 `fillna` 落盘（grep `duplicated|drop_duplicates|dedup` 在该文件零命中）。唯一的中位数填充只作用于离群检测的**内存副本** `Xf`（`:1134-1136`），不落盘、不回写。唯一的数据表输出是独立文件 `outliers_detailed_report.csv`。
2. **「70% 门禁 + 强制禁用 / Zero-Masking」尚未实现**：`epigenetic_gating` 只把结果写进 `blocked_channels` 与 Markdown 文案；全仓库 grep `blocked_channels` 的消费者只有 `frontend/src/components/QcDashboard.tsx:21`（且它读的是 `qc_pass` 而非 `blocked_channels`）。`Zero-Masking` 一词只出现在 `data_QC.py:1086/1255/1431` 的文案里，`src/input_control/cell_environment_combination.py` 中的 masking 是**通道选择掩码**（选中即保留原始值、未选中即置零），不是「整通道缺失时的降级填充」。
3. **`analyse/config.py:47` 的 `ambiguous_detection_only=True`** 表达了「只检测，处理策略由 Wizard 决策」的设计意图，但该 dataclass 未被 `data_QC.py` 引用（`data_QC.py:27-47` 无 `analyse.config` import）；`AnalysisConfig` 的实际消费者是 `analyse/evidence/*` 与 `analyse/pipeline.py`。
4. **`workflow_config.json` 目前只是一个占位符，不存在写入机制**：全仓库仅 3 处提及——`backend/crispr_workspace/project.py:64`（`config_versions` 中恒为 `None` 的键）、`frontend/src/views/WorkspaceView.tsx:104,106`（文案）、`docs/frontend_architecture.md:61`（文档）。**没有任何代码写出 `workflow_config.json`。**
5. **已实现的「决策」路径只有指纹校验**：`QCSessionManager.start/store`（`backend/crispr_workspace/qc_service.py:92-114`）落 `session_manifest.json`（`schema: "qc.session/1"`、`status`、`dataset.fingerprint`、`outputs`）；`create_from_qc`（`backend/crispr_workspace/project.py:113-131`）要求 `status == "completed"`，且当传入 `current_paths` 时比对指纹、不一致即 `ValueError("数据集已变化, 不能复用旧 QC 结果; 请先重新运行 QC")`。这是「不得换数据后误用旧 QC」的硬约束，与「用户勾选禁用通道」无关。
6. `analyse/pipeline.py:133-134` 中任务 `qc` 的「执行」是空操作（`if _should_run("qc"): _finish("qc")`），批次级数据质量内容来自 `build_data_quality_md`（`analyse/reports/markdown_report.py:152-165`），与 `data_QC.py` 是两套不同层级的东西。
...
**A. 严格编码器（`src/feature_engineering.py`，数据实际来源）**
...
- 线性回归侧另有「哑变量陷阱防护」：`select_non_t_reference_features`（`src/linear_regression/linear_regression.py:468-488`）剔除所有以 `_T` 结尾的特征列（`REFERENCE_T_SUFFIX = "_T"`，`:465`），`184 → 161`，以 `T` 为基准对照（`:734-744`）；实测 `results/batch_20260909_full/single_hct116_linear_all/linear_regression_info.txt` 记录 `Feature count: 161 / Numerical rank: 149 / Condition number: 565.6`。**特征张量本身始终保留 T 通道**（`src/feature_engineering.py:372-374`）。

**B. 宽松编码器（`Input/backend_runner.convert_raw_csv_to_npy`，向导路径）**

- 序列通道索引硬编码 `{'A':0,'C':1,'G':2,'T':3}`（`Input/backend_runner.py:61`），与 A 一致。
- 但序列先 `ljust(23,'N')[:23]`（`:65`），**不报错**；非 ACGT 字符不置位 → 该位点形成全零列（`:66-69`）。

## 4.2 表观/环境通道编码

**通道顺序（全局唯一约定）**：`A, C, G, T, CTCF, Dnase, H3K4me3, RRBS`，共 8 通道（`data/proceeded_data/feature_schema.json:53-62`、`data/feature_config.json:2-49`、`src/feature_engineering.py:147-152` + 配置顺序、`Input/backend_runner.py:107`）。环境通道顺序 = `feature_config.json` 中 `environment_features` 的**数组顺序**（`get_channel_specs`，`src/feature_engineering.py:794-804`），不是字母序。

**`per_position_binary` 语义**（当前 4 条轨道全部使用）：23 字符串逐位查 `encoding` 字典，**`"A" → 1.0`、`"N" → 0.0`**（`data/feature_config.json:13-16` 等）；出现字典外字符 → `ValueError`（`src/feature_engineering.py:471-478`）。输出 `(23,1)` float32。
...
**QC 侧的另一种读法**：`_parse_epi_vector` 把 `A/Y/T/1` 视为 1、其余（含 `N/0`）视为 0（`analyse/data_QC.py:236-238`），与 `feature_config.json` 的 `{A:1, N:0}` 一致；但 QC 的**离群特征向量只用 A/G/C 三个序列通道**（`:1112-1114`），与模型的 4 通道不一致。

## 4.3 去重 / 缺失 / 掩码 / 标准化

| 处理 | 是否存在 | 实现位置与语义 |
| :--- | :--- | :--- |
| **去重** | **仅特征工程有** | `remove_duplicate_rows`（`src/feature_engineering.py:1067-1126`）：`df.duplicated(keep="first")`，**仅删除整行完全相同的记录**；`load_source_csv` 在列检查与缺失检查之后调用（`:1218-1223`）。实测 hek293t `4566 → 2333`（删 2233），其余 3 系 0 条。**QC (`data_QC.py`) 不做任何去重**（grep 无命中）。 |
| **缺失值** | **特征工程 `ValueError` 中止；向导路径静默补零** | 特征工程：`df[required_columns].isnull().any().any()` 为真即抛 `ValueError` 并列出各列缺失计数（`:1187-1212`）。向导路径：匹配不到表观列 / 长度≠23 / 非数值 → 该通道保持 0（`Input/backend_runner.py:74-82`），目标列缺失 → 随机标签（`:85`）。QC：只统计不处理。 |
| **掩码 (masking)** | **有，但只是通道选择** | `create_channel_mask(schema, combination, selected_environments)`（`src/input_control/cell_environment_combination.py:1063-1144`）：长度 = `channel_count` 的 float32 向量；序列通道**恒为 1**（`:1120-1128`），被选中的环境通道置 1。`apply_environment_combination`（`:1151-1236`）先校验 `X_3d.shape[1:] == (sequence_length, channel_count)` 且 `np.isfinite(X_3d).all()`（NaN/Inf → `ValueError`，`:1210-1216`），再返回 `X_3d.copy() * mask.reshape(1,1,C)`（原数组不修改，`:1228-1236`）。**未选中通道被置 0 而非删除**，故所有组合的特征维度恒为 `23*8 = 184`。 |
| **标准化 / 归一化** | **数据管线内不存在；仅在模型训练层以 opt-in 开关存在** | `src/feature_engineering.py`、`cell_line_division.py`、`cell_environment_combination.py`、`data_QC.py` 均无 scaler 拟合（QC 的 `StandardScaler` 只用于离群检测，`analyse/data_QC.py:1137`）。模型侧：`train.py:623 --use-scaler`（`action="store_true"`，**默认关闭**），经 `build_train_kwargs` 透传 `use_scaler`（`train.py:402`）。MLP：`scaler.fit_transform(X_train)` 后 `transform(X_valid/X_test)`（`src/mlp/mlp.py:555-563`，日志 `StandardScaler fitted on training data.`）；CNN 同理（`src/cnn/cnn.py:640-650`，把 `(N,L,C)` reshape 成 `(N*L,C)` 拟合，并落盘 `cnn_scaler.pkl`，`:790-794`）；线性回归：`fit_transform(X_train)` / `transform(X)`（`src/linear_regression/linear_regression.py:159-162`、`:275-278`），`use_scaler` 与权重一同存盘（`:312`、`:375-382`）。实测批次 `results/batch_20260909_full` 的 `*_info.txt` 全部为 `use_scaler: False`。 |
...
**Status: Uncertain（CLI 可达、数据实际由它产出，但运行时流水线不 import 它）**——证据：全仓库 grep `feature_engineering` 只命中 `Input/backend_runner.py:94/182`（那是**同名但独立**的 `run_feature_engineering_step`，其内部不 import 本模块）与本文件自身；但 `data/proceeded_data/feature_engineering_summary.csv` 的列只可能由本文件 `process_all_csv` 写出（§2.3）。
...
- `parse_args() / main()`（`:1967-2029`）：**有 argparse**，3 个参数：`--source-dir`（默认 `data/source_data`）、`--output-dir`（默认 `data/proceeded_data`）、`--config`（默认 `data/feature_config.json`）。**无 `--seed`、无 `--use-scaler`、无 `--contamination` 等参数**。

## 5.2 schema 与 shape 变化（真实数字）

```
data/source_data/{cl}.csv
...
`data/feature_config.json` / `data/proceeded_data/feature_schema.json` 的实际取值（逐键核对）：
...
`feature_schema.json` 顶层**没有** `random_seed` / `train_ratio` / `scaler` 等字段；`environment_features` 在 schema 中保留了完整 spec（含 `encoding` 与 `enabled`），而 `Input/backend_runner.py:108-114` 写出的 schema **只有 5 个键**（`sequence_length`/`channel_count`/`feature_count`/`channel_names`/`sequence_channels`），**没有 `feature_names`，也没有 `environment_features`**。

## 5.3 特征命名/顺序

- 展平顺序 = **位置主序**：`for position in 1..23: for channel in channel_names:`（`generate_vector_feature_names`，`src/feature_engineering.py:940-949`）。
- `channel_names` 顺序即张量第 3 维顺序：`A, C, G, T, CTCF, Dnase, H3K4me3, RRBS`。
- 训练侧 `train.py:build_feature_names`（`:306-322`）用同一规则重建 2D 特征名；若数量不匹配则整体降级为 `feature_0..feature_{n-1}`（`:317-318`）。
...
**主实现**：`src/input_control/cell_line_division.py`（**Status: Active**——`train.py:242` import 并调用 `divide_data`；`predict.py:242` import `discover_available_cell_lines/load_cell_line`、`:759` import `load_feature_schema`；`scripts/make_notebook.py:138` 亦引用）。
...
- **退化保护（实测触发）**：`split_all_cell_lines` 开头 `if len(datasets) == 1 or len(cell_lines) <= 1:` 直接返回 `split_single_cell_line(...)`，但**返回值仍是 `split_type="single"`**（`:214-217`）；而 `data_digging.py` 为 `all` 模式逐个细胞系单独调度（`build_command` 只传 `--cell-line`，`data_digging.py:313-314`），因此实测目录 `results/batch_20260909_full/all_linear_all_heldout_hela/` 的 info 中 `split_type: all` 但 `cell_lines: ['hela']`、`input_shape_train: [5670, 184]`（= floor(8101×0.7)），**实际是细胞系内 70/15/15 划分，并非跨细胞系泛化**。`all_cnn_all_heldout_hct116_kernel_3` 同理（`[2967,23,8]/[635,...]/[637,...]`）。
...
- `train.py` 侧：`prepare_split_data`（`:232-252`）→ `cld.divide_data`；`validate_split_result`（`:259-284`）只校验 `X_*_3d` 为 3D 且样本数与 `y` 一致；`prepare_model_data`（`:291-299`）→ `cell_environment_combination.prepare_train_valid_test`。
- `predict.py` 的 mixed 十折 CV 走 `_load_mixed_subset`（`predict.py:241-260`），直接 concatenate 各细胞系，**不复用 `divide_data`**；`--ultimate-cv-folds` 默认 `10`、`--ultimate-seed` 默认 `42`（`predict.py:746-748`）。
...
- `train.py:33-35`：`DEFAULT_TRAIN_RATIO = 0.70`、`DEFAULT_VALID_RATIO = 0.15`、`DEFAULT_TEST_RATIO = 0.15`；`train.py:619-621` CLI 默认同值。
- `data_digging.py:409-411`：同样 `0.70/0.15/0.15`。
- `validate_split_fractions`（`cell_line_division.py:89-95`）：三者必须 > 0 且和 ≈ 1（`atol=1e-5`）。`train.py:validate_split_configuration`（`:207-225`）另用 `atol=1e-8` 复核。
...
- 默认 `random_seed=42`（`cell_line_division.py:154/195/210/256/303`；`train.py:31 DEFAULT_RANDOM_SEED = 42`、`:622 --seed`）。
- `all`（模式②，各系独立划分）用 `random_seed + i` 逐个偏移（`cell_line_division.py:241`）。
- `data_digging.py:59 MIXED_SEEDS = [42, 43, 44, 45]`；`build_command` 对 non-mixed **强制 `random_seed = 42`**（`data_digging.py:275`），对 mixed 用 42/43/44/45（`:146`、`:160`）。`--mixed-seeds` 可覆盖（`:404`）。
- 每次实验的 seed 写入 `config["random_seed"]` 与模型 kwargs 的 `random_seed`/`seed` 双键（`train.py:400-401`）。
...
- 运行命名：`all_{model}_{environment}_heldout_{cell_line}`（`data_digging.py:172`；`train.py:164` 同规则）。
- 测试集 = 留出细胞系的**全部样本**（不切片，`:235`）。

## 6.3 每实验目录产物

目录命名（由调用方决定，`train.py:build_batch_directories:178-200`）：`{results}/{batch}/{run_name}`，`run_name` 规则见 `train.py:generate_run_name:156-171`（`single_{cl}_{model}_{env}_{ts}` / `all_{model}_{env}_heldout_{cl}_{ts}` / `mixed_{model}_{env}_{ts}`）；但 `data_digging` 传入固定 `--run-name`（`data_digging.py:281`），故实际目录名**无时间戳**。

实测目录内容（`results/batch_20260909_full/single_hct116_linear_all/`）：

| 文件 | 写出者 | 说明 |
| :--- | :--- | :--- |
| `linear_regression_info.txt` | `src/linear_regression/linear_regression.py:630` | 头部含 `Feature count / Numerical rank / Condition number / Scaler / pinv_rcond`，随后是完整 `config`（含 `channel_names`、`input_shape_train/valid/test`）与 Validation/Test 六项指标 |
...
其余模型的同类产物：`cnn_info.txt / cnn_metrics.json / cnn_validation_metrics.json / cnn_predictions.csv / cnn_validation_predictions.csv / cnn_training_history.csv`（`src/cnn/cnn.py:428-456`）、`mlp_*`（`src/mlp/mlp.py:352-380`）、`transformer_*`（`src/transformer/transformer.py:434-462`）、`xgboost_*`（`src/xgboost/xgboost.py:431-442`）。

**批次级产物**：`{batch}/summary/metrics_tables/all_experiments.csv`、`single_cell_line_result.csv`、`all_cell_line_result.csv`、`mixed_cell_line_result.csv`、`baseline.csv`（`analyse/collect_results.py:332-396`，输出目录 `:436`）；`{batch}/summary/anomaly_report.md`（`anomaly_treatment.py:444`）；`{batch}/summary/feature_importance/key_regulatory_biomarkers.csv`（`backend_runner.py:289`）；`{batch}/summary/赛道二_results.csv`（`predict.py:821`）与 `{batch}/summary/ultimate/`（默认目录 `predict.py:796`，模型/配置落盘 `:435-469`）。

**注意**：`info.txt` / `metrics.json` 由**各模型模块的 `train()`** 写出，`train.py` 本身不写这些文件；`train.py` 只负责组装 `config` 与 kwargs 并透传（`train.py:520-592`）。
...
┌──────────────────────────────────────────────────────────────────────────────────────┐
│ 原始 CSV                                                                              │
│   data/source_data/{hct116,hek293t,hela,hl60}.csv  (N, 10)                            │
│   Chromosome,Start,End,Strand,sgRNA,CTCF,Dnase,H3K4me3,RRBS,Normalized efficacy       │
│   环境轨道 = 23 字符 A/N 串                                                            │
└───────────────┬──────────────────────────────────────────────┬───────────────────────┘
                │ 路径 A（数据实际来源）                        │ 路径 B（向导运行时）
                ▼                                              ▼
   src/feature_engineering.py  (CLI)              Input/backend_runner.convert_raw_csv_to_npy
   ...
   ⑥ 落盘: {cl}_23x8.csv / {cl}_184.csv /           ⑤ 落盘: {cl}_features_23x8.npy /
      ...
      + feature_engineering_summary.csv             (并预拷贝 data/proceeded_data 基准文件)
                │                                              │
                └──────────────────┬───────────────────────────┘
                                   ▼
                  data/proceeded_data/   (N,23,8) + (N,184) + y + metadata + schema
                                   │
        ┌──────────────────────────┼───────────────────────────────┐
        │ 只读 QC                   │ 训练/评估                      │ 结果级 QC
        ▼                          ▼                               ▼
 analyse/data_QC.py        train.py                                analyse/collect_results.py
 11 个检测模块:             ├─ validate_feature_schema               → summary/metrics_tables/
  ...
  ambiguous_bases          │     cell_line_division.divide_data     ├─ analyse/anomaly_treatment.py
  monomorphic              │     single / all(LOCO|各系独立) / mixed │   ├─ 实验级: ΔR2 与 ΔRMSE 同号
  gc_content               │   → create_split_indices(seed=42)      │   └─ 数据级: |Weight| > 10.0
  length_consistency       │     0.70/0.15/0.15, floor 余数入 test └─ analyse/data/validation.py
  ...
  data/data_report/                ▼
   ...
   gc_content_kde.png              _predictions.csv / (linear)_weights.csv …
        │                          │
        └──► 仅展示/建议,           └──► data_digging.py 网格 (1346 个 run 实测)
             不修改任何数据             └─► predict.py: mixed 10 折 CV → ultimate/ 赛道二_results.csv
```
...
1. **`analyse/data_QC.py:5` 声称「供 7 步交互式向导 (GUI/CLI) 第二步调用」**：`Input/main_wizard.py` 第 2 步是「选择已测量数据与待预测基因组」（`:146`），全文件不含 `qc`/`data_QC`；真实调用方是 `backend/crispr_workspace/qc_service.py:116-133`（HTTP 后端）。另有 `backend/crispr_workspace/analysis.py:22` 处名为 `qc` 的任务，但 `analyse/pipeline.py:133-134` 对它是空操作。
2. **`src/feature_engineering.py:99-115` 的输出示例已过期**：docstring 写 `hct116_23x7.csv`、`hct116_161.csv`、`hct116_features_23x7.npy`、`hct116_features_161.npy`（7 通道 / 161 维）；同一文件 `:145-152` 的注释与全部代码逻辑、`data/feature_config.json`、`data/proceeded_data/feature_schema.json`、实测 `.npy` 均为 **8 通道 / 184 维**。161 维在仓库中的真实来源是**线性回归剔除 23 个 `_T` 列后的输入**（`src/linear_regression/linear_regression.py:468-488`）。
3. **`analyse/data_QC.py:12` 声称报告含「第 1,3,4,5,6,7,8 项」**：实际 `build_markdown` 写出 `## 1,3,4,5,6,7,8,9,10` —— 第 2 项确实缺席（与 docstring 一致），但多出 9/10 两节（补充 QC-A/QC-B），docstring 未提及。
4. **`src/input_control/cell_line_division.py:5` 声称「彻底去除写死的 4 大细胞系」**：`discover_available_cell_lines` 在目录为空时仍硬编码返回 `["hct116","hek293t","hela","hl60"]`（`:72`）。
...
7. **QC 的 70% 门禁文案承诺了未实现的强制行为**：`data_QC.py:1084-1087` 与 `:1428-1431` 称「系统将在特征工程阶段**强制禁用**该环境通道参与消融实验与训练，或必须降级为 Zero-Masking 模式」。`blocked_channels` 在仓库内**无任何消费方**（仅 `frontend/.../QcDashboard.tsx:21` 读 `qc_pass`），`src/feature_engineering.py` / `cell_environment_combination.py` / `train.py` 均不读 QC 结果；`Zero-Masking` 在代码中不存在实现，只有通道选择掩码。
8. **阈值没有集中管理**：`analyse/config.py:1` 声明「阈值不得散落各模块」，`QCConfig`（`:41-47`）给出 `strict_complete_gate=70.0`、`environment_missing_rate_limit=0.30`、`ambiguous_detection_only=True`；但 `analyse/data_QC.py` 未 import 该模块，70% 是**内联字面量**（`:1056`），`0.30` 缺失率上限与 `Eligible/Limited/Ineligible` 三态标签在全仓库**均无实现**（`Ineligible` 仅出现在 `analyse/reports/markdown_report.py:162` 的说明文字中）。
9. **两套结果级 QC 的配对规则不一致**：`analyse/data/validation.py:1-6` 明示「mixed 多 seed 绝不跨 seed 比较 (paired cohort 原则)」，其基线键含 `seed`（`:38-40`）；`analyse/anomaly_treatment.py:135-136` 的 `aggregate_metrics` 按 `(split_type, cell_line, model, environment)` 求均值、**丢弃 `random_seed`**，再据此比较 ΔR²/ΔRMSE（`:162-180`）。两者对同一批 mixed 结果可能给出不同判定。
10. **`workflow_config.json` 只有文档与前端文案，没有代码**：`docs/frontend_architecture.md:61`、`frontend/src/views/WorkspaceView.tsx:104,106` 描述「决策写 workflow_config.json，不写入原始数据」；后端仅有 `project.py:64` 的 `config_versions.workflow_config = None` 占位，**无任何写入函数**。当前唯一可验证的「决策落盘」是 QC 会话 manifest 与 `create_from_qc` 的指纹一致性校验（`qc_service.py:97-113`、`project.py:113-131`）。
11. **`backend_runner.convert_raw_csv_to_npy` 的目标列兜底会伪造标签**：目标列匹配失败时用 `np.random.uniform(0.5, 0.9, n)` 作为 `y`（`Input/backend_runner.py:85`），仅打印成功信息，不告警、不中止。
12. **两条编码路径的容错策略相反**：`src/feature_engineering.encode_sgrna` 遇到非 ACGT 立即 `ValueError`（`:411-418`），而 `Input/backend_runner.convert_raw_csv_to_npy` 补 `N` 并留下全零列（`:64-69`）——同一份含 `N` 的 CSV 在两条路径下行为完全不同（前者不可处理，后者静默通过）。
13. **`feature_engineering.py` 的模块输出示例称 7 通道，而 `Input/backend_runner` 的 schema 硬编码 8 通道**；同时后者把 `sequence_length` 取自向导 `seq_len`（`:110`），却把张量固定为 23（`:60`），二者可能不一致。
14. **`Input/backend_runner.py` 会覆盖用户数据来源**：`run_feature_engineering_step` 先把仓库基准 `data/proceeded_data/*.*` 拷入用户输出目录（`:100-105`），并对已存在同名 `{cl}_features_23x8.npy` 的细胞系**跳过转换**（`:130-133`）。因此当用户细胞系名与基准 4 系同名（默认向导值即为这 4 系，`main_wizard.py:32`）时，用户上传的数据可能被基准数据静默替换。
...
17. **`data_QC` 的 `--data auto` 默认指向仓库自带 `data/source_data`**（`_demo_data`，`:1666-1676`），而非用户数据；直接跑 CLI 会对仓库基准数据出报告。
18. **QC 离群检测的序列 One-Hot 只用 A/G/C**（`analyse/data_QC.py:1112-1114`，T 被省略），与模型输入 4 通道不一致；且 `StandardScaler` 在**全体样本**上拟合（`:1137`），若把该 `anomaly_score` 当模型特征会引入跨 train/test 的信息泄漏（当前仅作 QC 报告，不进入训练）。
19. **`outliers_detailed_report.csv` 的 `sample_index` 实为文件行号**（含表头偏移 +2，`:482`、`:1168`），并非 0-based 样本下标；列名与语义不符。
20. **QC 报告只统计「序列长度」而不校验 23**：`recommended_length_L` 取长度众数（`:722-723`），若数据整体是 20nt，报告会推荐 20nt，而下游 `encode_sgrna` 硬要求 23（`src/feature_engineering.py:346`）。
...
- [x] **序列编码完整 A/C/G/T** — 是。`DEFAULT_SEQUENCE_CHANNELS = ["A","C","G","T"]`（`src/feature_engineering.py:147-152`），`encode_sgrna` 对非 ACGT 抛错（`:411-418`）；`data/feature_config.json:2-7` 与 `feature_schema.json:3-8` 一致；`Input/backend_runner.py:61` 亦为 A0/C1/G2/T3。**但** QC 侧离群向量只用 A/G/C（`analyse/data_QC.py:1112-1114`），已在 §4.2/§6.5 标注。
...
- [x] **缺失 / 非标准碱基处理** — 特征工程：缺失值 `ValueError` 中止（`:1187-1212`），非 ACGT `ValueError`（`:411-418`）；向导路径：静默补 `N`/补零/随机标签（`backend_runner.py:64-85`）；QC：只检测不处理（`detect_ambiguous_bases:582-612`），`analyse/config.py:47 ambiguous_detection_only=True` 未被 QC 引擎引用。
...
- [x] **划分规则** — `single` 系内随机、`all` 分 LOCO（当前抛错）与各系独立后合并、`mixed` 先合并再随机；比例 0.70/0.15/0.15，`floor` 取整、余数归 test；seed 默认 42，mixed 网格用 42/43/44/45，non-mixed 强制 42（`cell_line_division.py:154-273`、`data_digging.py:59/275`）；LOCO 内部硬编码 0.85/0.15 且因 `test_fraction=0.0` 触发 `validate_split_fractions` 报错（`:225` + `:91-92` + `:155`）。
- [x] **seed** — 见上；`train.py:31/622`、`cell_line_division.py:154`、`data_digging.py:59`、`data_QC.py:60`、`predict.py:748` 均为可追溯的具体值。
- [x] **CLI 存在性** — `src/feature_engineering.py`（3 参数）、`analyse/data_QC.py`（3 参数）、`analyse/anomaly_treatment.py`（7 参数）**均有 argparse**；`src/input_control/cell_line_division.py`、`src/input_control/cell_environment_combination.py`、`analyse/data/validation.py` **均无 argparse**（后者仅 `if __name__ == "__main__"` 自测块位于 `cell_environment_combination.py:1765-1908`）。
- [x] **程序 Status** — `src/feature_engineering.py` = Uncertain（CLI 可达、产出仓库数据，但运行时无 import）；`cell_line_division.py` / `cell_environment_combination.py` / `data_QC.py` / `anomaly_treatment.py` / `data/validation.py` / `Input/backend_runner.py` = Active（均给出调用方与行号）；`data/todo_data.CSV` = Unused（无代码引用）。
...
> 文档范围：`train.py`（733 行）+ `src/{linear_regression,xgboost,mlp,cnn,transformer}/*.py` + `src/xai_importance.py`，
> 以及调用方 `data_digging.py` / `predict.py` / `backend/crispr_workspace/training.py`。
> 所有事实均来自实际源码，标注 `path:line`。**Status: 已完成**（仅阅读，未修改任何仓库文件）。

## 7.1 训练总入口 train.py (CLI→流程)

`train.py` 是"单次实验"的唯一统一入口：一次进程 = 一个 (model, split_type, environment, cell_line) 组合。
...
| 常量 | 值 | 位置 |
|---|---|---|
| `DEFAULT_DATA_DIR` | `data/proceeded_data` | train.py:26 |
| `DEFAULT_MODEL_DIR` / `DEFAULT_RESULTS_DIR` / `DEFAULT_LOGS_DIR` | `models` / `results` / `logs` | train.py:27-29 |
| `DEFAULT_BATCH_NAME` | `default` | train.py:30 |
| `DEFAULT_RANDOM_SEED` | `42` | train.py:31 |
| `DEFAULT_TRAIN_RATIO` / `VALID` / `TEST` | `0.70` / `0.15` / `0.15` | train.py:33-35 |
| `ALL_MODELS` | `["linear","xgboost","mlp","cnn","transformer"]` | train.py:37-43 |
| `VALID_SPLIT_TYPES` | `["single","all","mixed"]` | train.py:45-49 |
| `MODEL_MODULES` | linear/linear_regression → `src.linear_regression.linear_regression`；xgboost → `src.xgboost.xgboost`；mlp → `src.mlp.mlp`；cnn → `src.cnn.cnn`；transformer → `src.transformer.transformer` | train.py:56-63 |
...
| `--data-dir` | str | `data/proceeded_data` | 需含 `feature_schema.json` |
| `--batch-name` | str | `default` | 空/`.`/`none`/`flat` → 不建二级目录 (train.py:185-189) |
| `--run-name` | str | `None` | 缺省则由 `generate_run_name` 生成 |
| `--model-dir` / `--results-dir` / `--logs-dir` | str | `models` / `results` / `logs` | 输出根 |
| `--train-ratio` / `--valid-ratio` / `--test-ratio` | float | `0.70` / `0.15` / `0.15` | 三者之和必须 = 1 (train.py:223-225) |
| `--seed` | int | `42` | 透传给模型 `train(random_seed=/seed=)` |
| `--use-scaler` | store_true | `False` | 仅传标志，**train.py 不拟合 scaler** |
...
| `--compute-shap` | store_true | `False` | 仅 MLP `compute_shap` 生效 |
| `--shap-background-samples` | int | `100` | MLP DeepSHAP 背景样本上限 |
...
1. `sanitize_name` 归一化 model/split/environment（小写、空格与斜杠→`_`，train.py:70-74）。
2. `run_name` 缺省 → `generate_run_name`（train.py:480-486）。
3. `validate_feature_schema(data_dir)` 读取并校验 `feature_schema.json` 必含
   `sequence_length / channel_count / channel_names`（train.py:92-105）。
4. `load_model_train_function` 用 `importlib.import_module` 动态导入并取出模块级 `train`（train.py:112-131）。
5. `prepare_split_data` → `src.input_control.cell_line_division.divide_data(...)`，
   传 `train_fraction/validation_fraction/test_fraction/random_seed`（train.py:232-252）。
6. `validate_split_result` 校验 `X_{train,valid,test}_3d` 与 `y_*` 存在、3D、样本数一致（train.py:259-284）。
...
8. `validate_model_input_shape`：`{linear, linear_regression, xgboost, mlp}` 要求 2D，
   `{cnn, transformer}` 要求 3D（train.py:329-343）。
9. `build_feature_names`（train.py:306-322）：2D 时为 `pos{1..L}_{channel}`（按 channel_names 内层循环），
   长度与 `X.shape[1]` 不符则退化为 `feature_{i}`；3D 时直接返回 `channel_names`。
10. 组装 `config`（train.py:520-555）：run_name/model/model_module/split_type/cell_line(s)/environment/
...
11. `build_train_kwargs`（train.py:350-434）按被调 `train()` 的签名过滤：若模型 `train()` 有 `**kwargs`
    则整体透传；否则只保留签名中存在的键，并强制要求 `X_train,y_train,X_test,y_test` 四个核心键
    （train.py:429-432）。别名同时提供：`model_dir/model_dir_root`、`result_dir/results_dir`、
    `log_dir/logs_dir`、`random_seed/seed`、`sequence_kernel(_size)`、`environment_kernel(_size)`。
12. `result = train_function(**train_kwargs)`，返回 `{"run_name","config","result"}`（train.py:591-592）。

### 7.1.4 scaler 的 fit-on-train 证据（train.py 只传标志，不拟合）

- `train.py` 全文没有 `StandardScaler`/`fit_transform` 调用；仅把 `use_scaler` 放进 config
  （train.py:541）与 kwargs（train.py:402、572），最终由 `execute_args` 传 `use_scaler=args.use_scaler`
  （train.py:703）。
...
- 批次目录：`build_batch_directories` 生成 `models/<batch>`、`results/<batch>`、`logs/<batch>`
  （train.py:178-200）；batch 为空或 `./none/flat` 时直接用根目录。
- 单实验子目录再由各模型 `train()` 拼 `os.path.join(<root>, run_name)`（如 linear_regression.py:699-705）。
- `generate_run_name`（train.py:156-171）：
...
- 实际批量运行时调用方**显式传 `--run-name`**（data_digging.py:287），因此真实实验目录名不带时间戳，
  CNN 额外带 `_kernel_{k}`（data_digging.py:178-179）。仓库实测目录如
  `results/batch_20260909_full/all_cnn_all_heldout_hct116_kernel_3/`。
...
映射见 train.py:56-63；`load_model_train_function` 只要求模块暴露 `train` 属性（train.py:127-131）。

### 7.2.2 data_digging.py（Training Scope 网格）→ 子进程调用 train.py

- 常量：`MODELS=[linear,xgboost,mlp,transformer]`、`CNN_MODELS=[cnn]`、`CELL_LINES=[hct116,hek293t,hela,hl60]`、
  `MIXED_SEEDS=[42,43,44,45]`、`CNN_KERNELS=[3,5,7]`（data_digging.py:55-60）。
- `generate_experiments`（data_digging.py:125-164）：非 CNN 模型 × environment × {single(4 cell)、
  all(4 留一)、mixed(4 seed)}；CNN 再额外 × 3 个卷积核（153-162）。
- `build_run_name`（167-180）与 train.py 规则一致，但 mixed 带 `_seed_{s}`、CNN 带 `_kernel_{k}`；
  `build_command`（250-321）固定 `--run-name`，CNN 才追加
  `--sequence-kernel k --environment-kernel 3`（313），`--use-scaler` 可选（318-319）。
- 执行方式三选一：`subprocess.run([sys.executable, train.py, ...])`（324-338）；
...
`predict.py` 直接 `import` 模型类自行训练与预测，不复用 train.py 的 CLI/编排：

- `_build_torch_model`（predict.py:288-325）：`from src.mlp.mlp import MLPModel`（292）、
...
- `TrainingConfig`（37-88）只做 CLI 编排，明确"绝不 import/修改训练代码"（1-10）；
  `_dig_command`（137-167）拼 `data_digging.py` 参数（含 `--conv-channels1/2`，154），
  `_predict_command`（169-187）拼 `predict.py` 参数；`LocalRuntime.submit` 用子进程
  `start_new_session=True` 并落盘 `training_status.json`/`done.rc`（242-281）。
- `Input/main_wizard.py` 的 XAI 勾选框（324/332/340/348）仅存于 `xai_vars`，**不产生任何 CLI 参数**。
...
> `train.py` 不传 `params`（`build_train_kwargs` 的 all_kwargs 无该键，train.py:384-421），
> 因此网格训练**恒用上表默认值**；只有 predict.py 的 Ultimate 路径另建 `n_estimators/max_depth` 网格（predict.py:70-73）。
...
- **无置换检验**：模块 docstring 明确"置换检验已移除"（16），全仓库 grep `permutation` 仅命中
  `src/xai_importance.py:44` 的禁用字段清单。
...
| `compute_smoothgrad(model, X, device, n_samples=20, noise_level=0.15)` | 原生 SmoothGrad，返回 (mean_grad, std_grad) | 245-271 |
| `compute_mlp_robustness_importance(model, X_eval, feature_names, device)` | 组装白名单表 | 274-321 |
| `compute_and_save_shap(...)` | 可选 DeepSHAP（`shap.DeepExplainer`，背景样本 ≤100，`check_additivity=False`），写 `shap_values.npy` + `shap_global_importance.csv` | 422-474 |
...
- **SmoothGrad 细节**：`std_noise = 0.15 * (X.max()-X.min()+1e-12)`（259），
  循环 `n_samples=20` 次对 `x + N(0,std_noise)` 求梯度，返回均值与标准差（262-271）；
  调用处为 `n_samples=20`（297），**标准差被丢弃**（`_sg_std_mat`）。
- **导出列**：`IG_Mean = mean|IG|`、`IG_SNR = mean|IG| / (std(IG)+1e-12)`、
  `SmoothGrad_Mean = mean(|smoothgrad|)`（291-305）；再 rename `IG_Mean→MLP_IG`、`SmoothGrad_Mean→SmoothGrad`
  并白名单清洗（312-316），按 `MLP_IG` 降序（319）。
...
**注意：`train.py --conv-channels1/2`（32/64）与这些 filters 无关，实际恒为 64/64/128。**
...
- 网格端：`CNN_KERNELS=[3,5,7]`（data_digging.py:60），`build_command` 对 CNN 追加
  `--sequence-kernel k --environment-kernel 3`（data_digging.py:312-313），run_name 加 `_kernel_{k}`
  （data_digging.py:178-179）。
- Ultimate 端：`cnn33→seq_k=3`、`cnn53→5`、`cnn73→7`，环境核固定 3（predict.py:301-311）。
...
|---|---|---|---|
| `<M>_metrics.json`（linear: `linear_regression_metrics.json`） | linear 598-614 / xgb 431-433 / mlp 352-354 / cnn 428-430 / trans 434-436 | 测试集 6 指标 MSE/RMSE/MAE/R2/Pearson/Spearman | `analyse/collect_results.py`, `analyse/data/loaders.py:127-131`（排除 `validation` 名） |
...
| `<M>_info.txt` | linear 630-657 / xgb 459-492 / mlp 380-401 / cnn 456-477 / trans 462-483 | run_name、时间、配置字典、valid/test 指标（xgb 另有 best_iteration/best_score/超参） | `analyse/collect_results.py:38`, `analyse/data/loaders.py:115-125`, `data_digging.py:183-229`（续跑判定） |
...
| 模型文件 | linear `linear_regression_model.pkl`+`linear_regression_scaler.pkl`（300-324）；xgb `xgboost_model.json`+`xgboost_config.json`（244-263）；mlp/cnn/trans `<M>_model.pt`+`<M>_config.json`(+`<M>_scaler.pkl`)（mlp 700-711、cnn 787-798、trans 726-737） | 权重/state_dict + config | `predict.py` 不读这些文件（自行重训）；供人工/后续加载 |
| `training.log` | 各模块 `create_logger`（linear 491-509, xgb 120-138, mlp 70-88, cnn 65-82, trans 65-82） | 训练日志（文件+控制台） | 排障 |
| `shap_values.npy` / `shap_global_importance.csv` | mlp `compute_and_save_shap` 454-467（仅 `--compute-shap` 且 shap 可用） | DeepSHAP 原始值 npy + `Feature,mean_abs_shap` | 可选分析（**不在 XAI 白名单内**） |
| 汇总（非训练产出） | `analyse/collect_results.py:312-355` | `summary/metrics_tables/{all_experiments,single_cell_line_result,all_cell_line_result,mixed_cell_line_result,baseline}.csv` | GUI/可视化 |
| 汇总（非训练产出） | `analyse/importance_extraction.py` | `summary/feature_importance/{linear_coefficiency,xgboost_importance,mlp_importance,cnn33/cnn53/cnn73_importance,transformer_importance}.md` + `key_regulatory_biomarkers.csv` | `analyse/visualization.py:303-320`（星级掩码热图） |
| Ultimate（非 train.py） | `predict.py` 476-558 | `summary/ultimate/ultimate_{lr,mlp,cnn33,cnn53,cnn73,transformer}_model.*`、`ultimate_{xgboost}_model.pkl`、`ultimate_summary.json` | `predict.py` 生成 `summary/赛道二_results.csv` |
...
| MLP SmoothGrad | MLP | `compute_smoothgrad` mlp.py:245-271；调用 mlp.py:297（n=20, noise=0.15） | `SmoothGrad`（mean\|grad\|） | **否**（std 被丢弃） | **已实现**（README 所称 `SmoothGrad_SNR` 未实现） |
| MLP DeepSHAP | MLP | `compute_and_save_shap` mlp.py:422-474 | `shap_values.npy`,`shap_global_importance.csv`（**非白名单列**） | 否 | **已实现但可选**（需 `--compute-shap` + shap 库；GUI/data_digging 不传该参数） |
...
| SNR ★ 星级（★≥2.5 …） | 4 个非线性模型 | `analyse/importance_extraction.py:111-122, 249-262`（**不在 src/ 训练代码内**） | `.md` 报表 `sig` 列 | — | **已实现于后处理**；训练 CSV 不含星级 |
...
### `compute_smoothgrad(model, X, device, n_samples=20, noise_level=0.15)` — mlp.py:245
- 输入同上；输出 `(mean_grad (N,D), std_grad (N,D))`。
- 噪声尺度 `std_noise = noise_level × (X.max() − X.min() + 1e-12)`（全矩阵同一标量，259）；
  循环 n_samples 次：`x_noisy = x + N(0, std_noise)`，求 `Σŷ` 对输入的梯度（262-267）；
  返回逐元素均值与标准差。调用处仅用均值（297-298）。

### `compute_mlp_robustness_importance(model, X_eval, feature_names, device)` — mlp.py:274
- `MLP_IG = mean|IG|`、`IG_SNR = mean|IG| / (std(IG)+1e-12)`、`SmoothGrad = mean(|mean_grad|)`（290-298）；
  `export_feature_table("mlp", rename={"IG_Mean":"MLP_IG","SmoothGrad_Mean":"SmoothGrad"})` 后按 `MLP_IG` 降序。
...
> 注意：CNN 的 IG、MLP 的 SmoothGrad 都算了 std 但**没有对应的 SNR 导出列**
> （cnn.py:366 `_ig_std`、mlp.py:297 `_sg_std_mat`）。

**阈值/星级使用位置（不在训练代码中）**：
- `analyse/importance_extraction.py:111-122` `get_snr_significance_code`：
...
- 线性模型改走 FDR 星级 `_sig_for_fdr`（239-246：`<0.001/0.01/0.05/0.10`）；
  ...
  由 `analyse/visualization.py:303-320` 解析后用于热图显著性掩码（446）。
- `analyse/schemas.py:9` 与 `analyse/evidence/rules.py:36-44` 明确"SNR 不是 p 值"，
...
- 旧名兼容 `LEGACY_RENAME_MAP`（48-73）：如 `weight/coefficient/coef→Linear_Coefficient`、
  ...
  `ig_mean/mean_ig→MLP_IG`、`smoothgrad_mean/sg_mean→SmoothGrad`、
...
- 实测导出表头验证：`Feature,Linear_Coefficient,SE,t_stat,p_value,FDR`（linear）/
  ...
  `Feature,MLP_IG,SmoothGrad,IG_SNR`（mlp）/
...
└── batch_20260909_full/                 # --batch-name batch_20260909_full（1346 个实验目录）
    ...
    └── summary/                          # 由 analyse/ 与 predict.py 产生，非 train.py
...
| `results/<batch>/summary/metrics_tables/*` | `analyse/collect_results.py:312-355` |
| `results/<batch>/summary/feature_importance/*` | `analyse/importance_extraction.py`（各 `extract_*_importance`） |
| `results/<batch>/summary/ultimate/*` | `predict.py:542-558` + `_save_ultimate_model`（434-473） |
| `results/<batch>/summary/赛道二_results.csv` | `predict.py:821`（`generate_track2_results_ultimate`, 651+） |
| `workspace/runs/<kind>_<ts>/{training_status.json,plan.json,run.log,submit.sh}` | `backend/crispr_workspace/training.py:242-281, 371-390` |
...
| 2 | README:215-218 | 非线性模型统一 `SNR ≥ 2.5 (***)` | 训练侧（src/）无任何 SNR 星级逻辑；阈值只存在于后处理 `analyse/importance_extraction.py:111-122`，且报告里是 `.md` 的 `sig` 列而非训练 CSV 列 |
| 3 | README:216 | MLP 指标为 `IG_Mean, IG_SNR, SmoothGrad_SNR` | 白名单为 `MLP_IG, SmoothGrad, IG_SNR`；代码只导出 SmoothGrad 均值（mlp.py:297-304），**无 SmoothGrad_SNR** |
...
| 7 | README:196 / 图 | "[Step 3] importance_extraction.py 提取 p-val/FDR/TreeSHAP/ISM" | 说明性文字，实际还含 MLP IG/SmoothGrad、CNN IG、Transformer 注意力/熵，且非线性模型被强制禁止 p/FDR |
...
| 10 | train.py:636-641 vs cnn.py:517-519 | CLI 暴露 `--conv-channels1/--conv-channels2`（默认 32/64），并写入 config 与 `*_info.txt` | 五个 `train()` 均不接受这两个键，`build_train_kwargs` 按签名过滤（train.py:428）后**静默丢弃**；CNN 实际恒用 `sequence_filters=64, environment_filters=64, fusion_filters=128`（cnn.py:96-98、517-519）。`--hidden-dim2` 对 CNN 同样无效 |
| 11 | train.py:622 / data_digging.py:275 | `--seed` 号称控制随机性 | Linear `train(random_seed=None)` 收到后**从未使用**（linear_regression.py:697）；XGBoost 仅设 `random_state`（无全局 seed）；只有 MLP/CNN/Transformer 调 `set_seed`（mlp.py:511、cnn.py:528、transformer.py:533） |
| 12 | train.py:540-541 + CLI `--use-scaler` | 通用 scaler 开关 | XGBoost `train(use_scaler=False)` 形参从未被引用（xgboost.py:522）→ 对 XGBoost **无效**；Linear 的 scaler 在 `fit` 内拟合（159-163）而非 train.py |
| 13 | README:196 流程图 "Step 1 data_digging.py" | 暗示统一走 data_digging→train.py | `predict.py` **完全不调用 train.py**，自带 torch 训练循环（AdamW、batch 256、lr 1e-3、epochs=15、无早停，predict.py:328-351），与 train.py 路径（Adam、batch 64、epochs 100、patience 20）超参不同 |
| 14 | Input/main_wizard.py:324-348 XAI 勾选框（含"计算 TreeSHAP 归因信噪比"） | 暗示可开关 XAI | 复选框只写入 `form_data["xai_vars"]`，不生成任何 CLI 参数；训练路径的 XAI **无条件执行**（mlp.py:681、cnn.py:768、transformer.py:705、xgboost.py:609）；`--compute-shap` 仅控制 MLP DeepSHAP 且 data_digging/back-end 从不传递（全仓库 grep 无命中） |
| 15 | train.py:376-377 | conv_channels 默认 32/64 作为"模型超参"出现在每个实验的 config | 该字段会写入 `cnn_info.txt`，但对应层宽实为 64/64/128，**artifact 记录与实际结构不一致** |
...
| 17 | 当前 checkout 状态 | README 描述 `models/`、`logs/` 产物 | `models/`、`logs/` 为空目录，仓库仅保留 `results/batch_20260909_full/`（1346 个实验目录），无法据此核验模型文件与 `training.log` 的真实内容 |
...
- [x] **五个模型全部覆盖**：`linear`(7.3)、`xgboost`(7.4)、`mlp`(7.5)、`cnn`(7.6)、`transformer`(7.7)，
      每个都有 class/函数清单 + 训练循环精确超参 + 输出表；`MODEL_MODULES` 映射（train.py:56-63）已列出。
...
- [x] **SNR 公式**（8.3）四条均为源码原式并标注 `path:line`；星级阈值定位到
      `analyse/importance_extraction.py:111-122`（训练侧无阈值）。
...
> 判定口径: **Active** = 有实际调用方 (训练端 `Input/backend_runner.py` / 向导 / 引擎 / 文档命令);
> **Legacy** = 保留原位、被训练端以旧语义调用但引擎不使用; **Unused** = 定义后无引用;
> **Uncertain** = 代码存在但无法从仓库确定是否执行。
> 实跑产物取自唯一真实批次 `results/batch_20260909_full/`（1344 组实验）。

## 11.1 analyse/collect_results.py (441 行) — 指标汇总表生成

| 项 | 内容 |
|---|---|
| 状态 | **Active**（`Input/backend_runner.py:244-252` Step3 以 `--batch-dir` + `--split-types` 调用; `HPC_EXPERIMENT_PROTOCOL.md:77`; `README.md:145`） |
...
### 实验发现与解析规则
- `collect_batch()` (`analyse/collect_results.py:147-167`)：只遍历 `batch_dir.iterdir()` 的**直接子目录**，跳过名为 `summary` 的目录；发现门槛是 `glob("*info*.txt")` 或 `glob("*metrics*.json")` 非空 (`:155`)；单目录解析异常只打印 `[Warning]` 继续，无记录则 `RuntimeError` (`:162-163`)。
...
- `calculate_delta_R2()` (`:217-244`)：以 `environment == "sequence"` 的行为基线，键为 `(split_type, cell_line, model)`（`:227`），逐行 `delta_R2 = R2 - baseline`；sequence 行置 0.0。**该键不含 `random_seed`**，mixed 多 seed 时 `baseline_map[key]` 会被后写覆盖（`:228`），与引擎同 seed 配对纪律 (`analyse/data/validation.py:37-40`) 语义不同 —— 属旧版设计。
...
| 项 | 内容 |
|---|---|
| 状态 | **Active**（`Input/backend_runner.py:264-270` Step5 `--batch_dir`; `scripts/make_notebook.py:262` 直接 import） |
...
- `XAI_WHITELIST` (`:133-139`)：linear = `Linear_Coefficient, SE, t_stat, p_value, FDR`；xgboost = `XGB_Gain, XGB_Weight, XGB_Cover, TreeSHAP, SHAP_SNR`；mlp = `MLP_IG, SmoothGrad, IG_SNR`；cnn = `CNN_IG, CNN_ISM, ISM_SNR`；transformer = `Transformer_Attention, Attention_Entropy, Attention_SNR`。
...
| 项 | 内容 |
|---|---|
| 状态 | **Legacy（但仍在调用链上）**：新包 `analyse/visualization/` 同名遮蔽该文件，包内 PEP 562 `__getattr__` 惰性加载本文件并透传 `generate_all_visualizations` (`analyse/visualization/__init__.py:56-84`)，`Input/backend_runner.py:274-279` Step6 与 `HPC_EXPERIMENT_PROTOCOL.md:82` 仍走旧入口 |
...
| 项 | 内容 |
|---|---|
| 状态 | **Active**（`Input/backend_runner.py:255-262` Step4） |
...
> 补充：`analyse/visualize_results.py` 在 git 索引中存在但工作区**已删除**（`git ls-files` 列出、`ls` 缺失），仓库任何文档/代码均不再引用 → **已移除的 Legacy 文件**。
...
- 包版本 `__version__ = "0.1.0"` (`analyse/__init__.py:16`)；铁律为对训练系统只读、不反向 import (`analyse/__init__.py:13`)。
- CLI 入口：`python -m analyse.pipeline --batch-dir <batch> [--output <out>] [--analysis-plan plan.json]` (`analyse/pipeline.py:327-341`)。`--batch-dir` 必填；`--output` 缺省 `<batch>/analyse_out` (`:335`)；`--analysis-plan` 缺省 `default_plan()` (`:336`)。
- GUI/后端入口：`backend/crispr_workspace/analysis.py:202-204` 以子进程 `python -m analyse.pipeline --batch-dir … --output … --analysis-plan …` 调用，任务勾选由 registry 目录渲染，状态读 `analysis_status.json`。
- 编排主函数 `run_analysis(batch_dir, output, plan, config) -> Dict` (`analyse/pipeline.py:64-324`)，只做编排，科学计算分布在各模块 (`:1`)。
- **阶段执行顺序**（源码顺序，非并行）：
...
- **三层状态语义** (`analyse/plans.py:1-7`)：`selected`（用户是否选）× `available`（数据/依赖是否支持）→ `status`（实际执行）。`ExecutionStatus` (`plans.py:18-24`) = `pending|running|completed|skipped|unavailable|failed`。约定：`skipped` 表示未选中或依赖不满足（validator 一律给 `skipped`，`:149-153`）；`unavailable` 表示选中但无 runner/能力不足且 reason 必填；`failed` 表示执行异常。
...
| | `bootstrap_ci_exclude_zero_alpha` | 0.05 | `:35` |
| | `cellline_consistent_ratio` | 0.75 | `:36` |
| | `environment_strong_effect` | 0.01 | `:37` |
...
- 实际被生产代码消费的阈值仅 4 个：`snr_threshold/snr_moderate/snr_weak/min_effect_size`（`evidence/rules.py:41-52`）、`unstable_effect_threshold`/`environment_strong_effect`/`min_coverage`/`direction_concordance`（`evidence/integration.py:77-78,132-133,145`）。`QCConfig` 三个字段、`fdr_nominal`、`cellline_consistent_ratio`、`bootstrap_ci_exclude_zero_alpha`、`bootstrap_iterations/permutation_iterations` **当前无消费点**（`grep` 全仓 0 命中）→ 预留配置。
- `AnalysisConfig.as_dict()` 用 `dataclasses.asdict` (`:63-65`)，键名为 `bootstrap_iterations / bootstrap_seed / permutation_iterations / random_seed`（**与 `analyse/docs/interface_contract.md:44-46` 写的 `bootstrap_n/permutation_n` 不一致**）。
...
- registry 的消费者是后端 GUI 目录（`backend/crispr_workspace/analysis.py:59-63`），**pipeline 内部不读 registry**（阶段是硬编码顺序）。
...
- 产出摘要 `analyse/attribution/summary.py`：`top_attribution_features(table, top_n=10)` (`:9-26`) 按 `(model, method, feature)` 求 `mean|importance|`/`mean importance`/`n_contexts`，再每 `(model, method)` 取 top-N；`build_motif_summary_md` (`:29-50`) 写 04 md，末尾三条声明：本表是 importance/attribution 证据、不做统计显著标签；ISM 是核苷酸替换效应；motif discovery/enrichment/known-motif 比较由后续 Phase 提供（当前 unavailable）(`:47-49`)。**`:35-36` 计算的 `coverage` DataFrame 未被使用（死代码）**。

## 12.10 cell-line 一致性

- `analyse/cellline/consistency.py`：模块 docstring 明确"不做 predictive R² 不同 = biological mechanism 不同式解读" (`:3-5`)。
...
- **一致性判定规则** `analyse/evidence/integration.py:41-62`：先剔除非有限与 0 方向 (`:52-53`)；**已知方向 < 2 → `("Uncertain", None)`** (`:54-55`)；存在任何少数方向 → `"Context-conflicting"`（返回多数方向，`:58-59`）；多数占比 ≥ `consistent_ratio(0.75)` → `"Context-consistent"`（`:60-61`）；否则 `"Context-dependent"`（`:62`）。
...
- `environment_evidence_matrix(main_effects, cellline_summary=None, config)` (`:112-182`)：
  ...
  - 先按 `(model, environment)` 求均值 (`:134-135`)，再逐 factor：`finite` = 有效效应模型集 (`:138-139`)；`concordance`；`strong = any(|e| >= environment_strong_effect(0.01))` (`:145-146`)；`overall_effect = mean(finite)` (`:147`)；`supporting` = 与总体多数符号相同的模型数 (`:149-151`)。
  - **Context 冲突降级**：若该 factor 在 cellline_summary 中的 `context_label` 众数为 `"Context-conflicting"` → 直接 `INCONCLUSIVE`，不再走 Tier 判据 (`:152-159`)。
  - 输出列：`feature, feature_type="environment", applicable_models, coverage, direction_concordance, cell_line_consistency, overall_effect, model_effects, unstable_rows_excluded, evidence_tier`。
...
- 兼容桥 (`__init__.py:56-84`)：`_load_legacy_viz_module()` 以 `importlib.util.spec_from_file_location("analyse._legacy_viz_module", analyse/visualization.py)` 惰性加载旧模块；`__getattr__("generate_all_visualizations")` 透传；其它属性抛 `AttributeError`。

## 12.13 产物 schema（实跑 `results/batch_20260909_full/analyse_out/`）

### 目录
`tables/`、`summary/`、`figures/` + `analysis_plan.json`、`analysis_status.json`、`execution_log.json`（`PipelineArtifacts.create`, `pipeline.py:35-40`；GUI 运行还会额外生成 `analysis_run.json`/`analysis_run.log`，见 `backend/crispr_workspace/analysis.py:186-206`，非引擎产物）。
...
```bash
## 产物清单与规模
find results/batch_20260909_full/analyse_out -type f | sort
wc -l results/batch_20260909_full/analyse_out/tables/*.csv
awk 'END{print NR-1}' results/batch_20260909_full/analyse_out/tables/attribution_summary.csv   # 331200
ls results/batch_20260909_full/analyse_out/figures/*/*.png | wc -l                            # 15
## 不稳定隔离与字段语义
python3 - <<'EOF'
import csv,collections
rows=list(csv.DictReader(open('results/batch_20260909_full/analyse_out/tables/environment_main_effects.csv')))
...
EOF
## 测试规模与死码审计
grep -c 'def test_' analyse/tests/test_*.py
grep -rn "<symbol>" --include="*.py" analyse/ backend/          # 逐个未使用符号核验
```
...
- 运行方式（`analyse/README.md:55`）：`python -m unittest discover -s analyse/tests`（或单文件 `python analyse/tests/test_core.py`，文件末尾自带 `unittest.main(verbosity=2)`）。共 **31 个 test 方法**：test_core 20 / test_environment 4 / test_attribution 3 / test_phase5 3 / test_phase6 1（`grep -c "def test_"` 实测）。
- `test_core.py`（`analyse/tests/test_core.py`）：
...
- 未覆盖项（grep 实证）：`stats/hypothesis_tests.py`（permutation / ANOVA 占位）、`environment.compute_pair_interactions`、`evidence.rules.classify_mutation_effect`、`data.loaders.iter_experiment_records`、`attribution.columns.pick_column` 均无单测（**与 `analyse/docs/code_cleanup_report.md:22,25` 声称"函数有独立单元测试/模块有单测"不符**）。
...
|---|---|---|---|---|
| 1 | 未实现 | `motif_discovery` 无 runner，选中后状态 `unavailable`（reason: analysis module not yet implemented in this build (phase roadmap)） | `analyse/README.md:73` 把 "Phase 4 attribution 统一抽取 + motif" 标为 ✅，而 `:74` "待续" 才列出 motif discovery runner；`SequencePlan.motif_discovery` 默认 True（`:42`） | `analyse/pipeline.py:241-247`；`results/.../analysis_status.json`；`analyse/docs/workflow_architecture.md:61` 亦承认其 unavailable |
| 2 | 未实现（占位） | `anova_interface()` 恒返回 `available=False` | interface_contract §7 "ANOVA/Shapley runner 就绪前状态 unavailable"（一致），但 registry 注释称"需要足够 factorial observations"暗示可运行 | `analyse/stats/hypothesis_tests.py:56-73`; `analyse/registry.py:60-63` |
| 3 | 未实现 | `bootstrap`/`hypothesis_testing`/`fdr_correction` 三个统计任务恒 `unavailable`（无 runner 挂载），尽管 `stats/` 四个模块已实现且有部分单测 | `docs/workflow_architecture.md:2 表格` 把 stats 层描述为已实现能力；`analyse/README.md:36-37` 标注为 roadmap（一致） | `analyse/registry.py:70-71`（runner=None）；`analyse/pipeline.py:241-247` |
| 4 | 死规则 | `classify_cellline_consistency` 的 `"Context-dependent"` 分支不可达（方向全同→consistent，方向不全同→conflicting） | docstring 与 interface_contract §6 均把 Context-dependent 列为正常标签；`CellLineEffect.context_label` 注释同样列出 | `analyse/evidence/integration.py:52-62`；实测 05 md 标签仅 conflicting/uncertain/consistent（`results/.../summary/05_cellline_heterogeneity.md`） |
| 5 | 字段语义错 | `evidence_matrix.unstable_rows_excluded` 实为"该 factor 被 groupby 折叠掉的行数"（63 行 → 7 模型行 ⇒ 56），真实不稳定行每因子仅 7 行 | `docs/workflow_architecture.md:5` 与 08 md 把它当"被隔离的发散上下文数（28）" | `analyse/evidence/integration.py:140-141`；实测 `unstable_rows_excluded=56`/因子，而 pipeline 独立统计的 `matrix_excluded=28`（`pipeline.py:218-221`） |
| 6 | 配置未接线 | `cellline_consistent_ratio=0.75`、`QCConfig.*`、`fdr_nominal`、`bootstrap_iterations`、`permutation_iterations`、`bootstrap_ci_exclude_zero_alpha` 无消费点；`summarize_environment_by_cellline` 用函数默认值 0.75 而非 config | docs 称"阈值一律来自 config，不散落" | `analyse/evidence/rules.py:1`；`analyse/pipeline.py:195`；`grep` 0 命中 |
| 7 | 状态/reason 错配 | completed 任务的 `reason` 会保留 validator 旧文案（如 `"environment features unavailable"`、`"need >=2 cell lines"`） | 约定 "unavailable=选中但缺能力 (reason 必填)"，未定义 completed 携带 reason | `analyse/pipeline.py:109-116`；实测 `analysis_status.json` |
| 8 | 文档 schema 与产物不符 | 实际 `analysis_status.json` 无 `engine_version/batch/config`，task 内无 `completed_at`；这些字段只存在于**返回值** `status_dict`（供 00 md 使用） | `analyse/docs/interface_contract.md:41-53` 把这些字段写进输出 schema（`:45` engine_version、`:49` `bootstrap_n/permutation_n/permutation_seed`、`:53` task.completed_at） | `analyse/pipeline.py:254-261`; `analyse/plans.py:220-235`; 实测 JSON |
| 9 | 行数/规模数字 | `attribution_summary.csv` 实测 **331 200** 行；`figures/` 实测 15 张 PNG；单测 31 项 | `analyse/README.md:44`、`analyse/docs/workflow_architecture.md:71` 写 "287k 行" | `awk 'END{print NR-1}'`；`ls figures/*/*.png | wc -l` |
| 10 | 图形规格 | `save_figure(dpi=150)`、`plt.subplots(dpi=150)` | `analyse/README.md:36`、`analyse/docs/workflow_architecture.md:20`、`visualization/__init__.py:1` 声称 "300dpi" | `analyse/visualization/core.py:16-21`；`environment_plots.py:25` |
| 11 | 版本串 | 同文件头 "V6" 与运行打印 "启动全景生信绘图引擎 (V4)" | 二者互斥 | `analyse/visualization.py:3` vs `:916` |
...
| 13 | 注释声称的单测 | `code_cleanup_report.md:28` 称 `compute_pair_interactions` "有独立单元测试"、`:29` 称 `stats/{bootstrap,hypothesis_tests,…}` "模块有单测" | `test_environment.py` 无该函数引用；`hypothesis_tests` 无任何测试引用 | `grep -rn compute_pair_interactions analyse/tests` 0 命中 |
...
| 17 | 新引擎无发散过滤 | `prediction_summary.csv` 中 linear 的 `R2_mean ≈ -1.87e16`（含发散实验）；引擎只做"隔离不删除"（且隔离仅作用于证据矩阵），预测/归因表不过滤 | 文档强调数值稳定性护栏 | `analyse/data/loaders.py`（无 filter）；实测 `02_prediction_generalization.md` linear 行 |
| 18 | Capabilities 非数据推导 | `n_environment_factors=4`、`environment_factorial_observations=16`、`replication_per_cellline=4` 硬编码，即使表无环境特征且 `has_environment_cols=True` 默认 | `plans.py:117` "由实际数据推导的可用性" | `analyse/pipeline.py:43-61,88` |
| 19 | 已删除的 legacy 文件 | `analyse/visualize_results.py` 在 git 索引中、工作区不存在，且无任何引用 | 无文档提及 | `git ls-files Submit/analyse`；`ls` 缺失 |
...
- **tests**：§13 覆盖 5 个测试文件 31 个用例的断言要点、运行命令（`python -m unittest discover -s analyse/tests`）与未覆盖清单（hypothesis_tests / compute_pair_interactions / classify_mutation_effect / iter_experiment_records）。
...
- **未使用/不确定标注**：Active/Legacy/Unused/Uncertain 均在 §11 各表与 §12 各节显式给出（如 `visualization.py` 为 Legacy-but-active-by-bridge；`summary/plots/` 本批缺失 → Uncertain；`analyse/visualize_results.py` 已删除）。
...
| 文件 | 唯一职责 | 证据 |
|---|---|---|
| `predict.py` | mixed 已测池 10 折 CV 选超参 → 全量重训 Ultimate 模型 → 对【目标待测数据集】真实预测 → 写 `赛道二_results.csv` | `predict.py:1-24` (docstring), `predict.py:757-840` (`main`) |
| `data_digging.py` | 已测数据 Training Scope 网格实验 (逐实验 subprocess 调 `train.py`) | `data_digging.py:1-17`, `data_digging.py:436-514` |

两者都不再做对方的旧职责 (拆分注释: `predict.py:5-9`, `data_digging.py:5-10`)。`predict.py` 中仍残留旧职责的两个函数 `train_ultimate_models` (`predict.py:476-559`) 与 `generate_track2_results_ultimate` (`predict.py:651-716`),**在仓库内无任何调用点**(`grep -rn` 全仓库仅命中定义行) → 标记 **Legacy / Unused**;`main()` 走的是 `run_ultimate_with_resume` (`predict.py:931-1003`) + `_write_track2_from_preds` (`predict.py:892-928`)。

## 15.2 predict.py CLI 参数表 (`parse_args`, predict.py:723-754)

| 参数 | 类型/取值 | 默认 | 作用 |
|---|---|---|---|
| `--data-dir` | str | `data/proceeded_data` | 已测数据 + `feature_schema.json` 根 |
| `--results-dir` | str | `results` | 结果根 |
| `--batch-name` | str | `""` | 空 → 直接 `results/summary/`; 非空 → `results/[batch]/summary/` (`predict.py:793-796`) |
| `--models` | nargs+, choices `linear xgboost mlp transformer cnn` | `None`(全部) | 终极模型; `cnn` 展开为 cnn33/53/73 (`predict.py:268-285`) |
...
| `--target-epigenetics` | nargs+ | `None` | 目标实际具备的表观通道; 缺省从目标文件列自动识别 (`predict.py:766-770`) |
| `--candidate-top-k` | int | `20` | 共识排序取 Top-K |
| `--ultimate-dir` | str | `None` | 缺省 `results/[batch]/summary/ultimate` (`predict.py:796`) |
| `--ultimate-cv-folds` | int | `10` | CV 折数 |
| `--ultimate-epochs` | int | `15` | torch 模型每折 epochs |
| `--ultimate-seed` | int | `42` | KFold/训练 seed |
| `--device` | str | `None` | 空则 torch.cuda 探测 (`predict.py:939-944`) |
| `--dry-run` | flag | False | 只打印模式/通道规划/数据量/模型计划, 不建目录不写文件 (`predict.py:798-815`) |
| `--generate-candidates` | flag | False | **已废弃无意义**, 代码中除 `add_argument` 外无任何引用 (`predict.py:752-753`) |
...
`main()` 断言 `X2.shape[1] == len(feature_names) == 23 * n_channels` (`predict.py:789-790`)。
**严格子集对齐**: 训练输入 (mixed) 与预测输入 (target) 用同一 `plan` 裁剪 —— pooled 模式用 schema 全通道 (`predict.py:779-782`), target 模式用目标实际通道。
...
**LR `_T` 参照列对齐 (parity)**: `_drop_lr_reference_columns` (`predict.py:354-362`) 调 `src.linear_regression.linear_regression.select_non_t_reference_features` (`src/linear_regression/linear_regression.py:468-488`), 剔除所有以 `_T` 结尾的特征列 (T 为基准, 184→161)。训练端 (`predict.py:511-512`)、预测端 (`predict.py:669`, `884`)、汇总端 (`predict.py:900-902` 复用 progress 中保存的 `lr_names`) 三处一致。若无 `_T` 列 (7 通道旧数据) 原样返回。

`_save_ultimate_model` (`predict.py:434-473`): lr → `ultimate_lr_model.json` (含 `weights/std_errors/t_stats/use_scaler/feature_names`); xgboost → `ultimate_xgboost_model.pkl` + `_config.json`; torch → `ultimate_<kind>_model.pt` (`{state_dict, config}`) + `_config.json`; 全部经 `_atomic_bytes` (`predict.py:426-431`, 同目录 `.tmp` + `os.replace`)。
...
| `run_ultimate_with_resume` | 931-1003 | 逐 kind: 已完成 → `np.load(row["preds_file"])` 复用并打印 `[Resume]`; 未完成 → CV 选参 + 全量重训 + `_save_ultimate_model` + `_preds_on_target` → `np.save` 到 `preds_<kind>.npy` (原子写, `predict.py:977-980`) → 追加/替换同 model 的 row (`predict.py:988-991`) |
| 结束 | 996-1003 | 写 `ultimate_summary.json` (models 直接取 progress rows); `preds` 为空 → `RuntimeError`; 否则 `_write_track2_from_preds` |

**明确事实 (代码注释 `predict.py:819-820, 845-849`)**: 完成判据是本 run 的 `ultimate/progress.json` 条目 + 原子落盘的 `preds_<kind>.npy`;**绝不扫描 `results/` 或 batch 目录** —— 未跑完的实验目录一律忽略, 中断后重跑同一命令只补"没有完成标记"的模型。断点续跑版汇总不再需要模型对象 (`_write_track2_from_preds`, `predict.py:892-928`)。

## 15.6 输出 `赛道二_results.csv`

写入者 `_write_track2_from_preds` (`predict.py:892-928`), 输出路径 `results/[batch]/summary/赛道二_results.csv` (`predict.py:821`), `utf-8-sig`。

- 共识: `usable = [cv_r2 > 0 的模型] or 全部` (`:895`), `consensus = mean(preds[usable])` (`:897`), `argsort` 降序取 Top-K (`:898`)。
- 列 (7 列, 与仓库实测文件一致): `训练方式_细胞系`(恒 `mixed_all`)、`候选编号`(`CAND_sgRNA_%03d`)、`位点`(`extract_locus_from_row`, `predict.py:566-605`, `chr(start~end)` 或从 gene 名正则抽取, 兜底 `chrUnknown(...)`)、`候选序列(23nt)`、`预测编辑效率`(`%.4f [Linear:0.36|XGBoost:0.42|...]` 逐模型分解)、`最显著正向促进特征 (Top Positive Drivers)`(`match_ultimate_drivers`, `predict.py:608-648`: 仅前 20nt、仅 LR 权重 > 0 的 `pos{p}_{base}`, 相对最大权重 0.8/0.5/0.2 → `***`/`**`/`*`/`.`)、`对应模型`(`Ultimate_Consensus (Linear+...)`)。
...
| `--data-dir` / `--model-dir` / `--results-dir` / `--logs-dir` | `data/proceeded_data` / `models` / `results` / `logs` | 透传 `train.py` |
...
| `run_experiment_in_process` | 341-362 | 改写 `sys.argv = [train.py] + cmd[2:]` 后 `import train; train.main()`; **仅限顺序执行** (sys.argv 进程全局); 异常吞掉返回 failed |
...
`train.py` 侧产物命名 (resume 依赖): run 目录内 `<model>_info.txt` (含 `run_name/model/split_type/cell_line/environment/combination/random_seed/sequence_kernel/...`) 与 `<model>_metrics.json`; 实测目录 `results/batch_20260909_full/all_cnn_all_heldout_hct116_kernel_3/{cnn_info.txt,cnn_metrics.json,cnn_validation_metrics.json,...}` 验证了该 glob 契约。`train.py` 打印 `[✓] Experiment <run_name> Finished Successfully.` (`train.py:724`)。
...
`CRISPRPlatformWizard(tk.Tk)` (`Input/main_wizard.py:22`), 窗口 880x700, `total_steps = 7` (`:70`)。骨架: header (标题 + `ttk.Progressbar(maximum=7)`) / `container` / footer (`btn_prev`, `btn_next`) (`:80-103`)。`_show_step(step)` (`:109-427`) 每步重建 container 内容; `_clear_content` (`:105-107`) 销毁全部子控件。
...
| 自动默认 | `measured_data_dir` 空 → backend_runner 回落 `data/proceeded_data` (`Input/backend_runner.py:178-180`); 无细胞系→`hct116`, 无模型→`linear`, 无划分→`single` (`:167-169`) |
...
| 步骤 | 打印 | 调用 | 细节 |
|---|---|---|---|
| 1/7 特征工程 | `:177` | `run_feature_engineering_step(form_data, raw_measured, proceeded_data_dir, active_cls)` (`:182`; 定义 `:94-141`) | raw 不存在 → 回落 `<repo>/data/proceeded_data`; 拷 benchmark 文件; 写 `feature_schema.json` (`channel_names=["A","C","G","T","CTCF","Dnase","H3K4me3","RRBS"]`, `:107-117`); 逐细胞系: 已有 `_features_23x8.npy` 跳过, 否则拷 benchmark → `convert_raw_csv_to_npy` |
...
| 2b/7 候选预测 (条件) | `:210` | `predict.py` (`:223-233`): `--batch-name ""`, `--data-dir`, `--results-dir`, `--models`, `--cell-lines`, `--target-input`, `--ultimate-dir <root>/ultimate`, 条件追加 `--target-epigenetics *target_available_epis` (`:235-236`) | 仅 `has_target`; `target_genome_dir` 为文件直接用, 为目录取第一个 `*.csv`/`*.CSV`, 无效则打印警告并跳过 (`:212-221`) |
| 3/7 指标收集 | `:244` | `analyse/collect_results.py --batch-dir <results> --split-types *active_splits` (`:246-252`) | 脚本不存在则跳过 |
| 4/7 异常检测 | `:255` | `analyse/anomaly_treatment.py --batch-dir <results>` (`:256-261`) | 生成 `summary/anomaly_report.md` |
| 5/7 重要性提取 | `:264` | `analyse/importance_extraction.py --batch_dir <results>` (`:265-270`) | **注意下划线参数名** (`:269`) |
...
包原则 (`backend/crispr_workspace/__init__.py:1-11`): 只做项目/配置/编排/启停/状态/日志与结果浏览; 科学计算一律子进程或只读产物;**零第三方运行时依赖**; 版本 `0.1.0`。静态守卫测试 `backend/tests/test_no_training_dependency.py` 禁止 `import src/train/predict/data_digging` 与顶层 `import analyse`。
...
`MANIFEST="session_manifest.json"`, `ENGINE=analyse/data_QC.py` (`:23-24`)。`_copy_inputs(src,dst)` (`:27-57`): 目录取 `csv/tsv`, 文件直用; 0 个 → `ValueError("no dataset file given")`; 单个直接返回; 多个 → 复制到 `<session>/inputs_src/` 并去重文件名。
...
|---|---|---|
| `kind` | `"dig"` | 选脚本: `dig`→data_digging.py / `predict`→predict.py (`:128-132`) |
...
| `train_ratio/valid_ratio/test_ratio` | 0.70/0.15/0.15 | **未出现在 dig 命令中** (build_command 未追加, 依赖 train.py 默认) |
| `use_scaler` | False | `--use-scaler` (仅真值追加) |
| `data_dir/results_dir/model_dir/logs_dir` | `""` → `default_dirs()` 填 `<repo>/data/proceeded_data`、`<repo>/results`、`<repo>/models`、`<repo>/logs` (`:93-102`) | `--data-dir --model-dir --results-dir --logs-dir` |
...
技术栈: React 18 + TS + Vite 5 + Tailwind 4 (`frontend/package.json:13-27`)。scripts: `dev`/`build`(`tsc --noEmit && vite build`)/`preview`/`typecheck`/`test`(`vitest run`) (`:6-12`)。`vite.config.ts`: port 5173, 代理 `/api → http://127.0.0.1:8765`(changeOrigin) (`vite.config.ts:7-15`)。API base 可由 `VITE_API_BASE` 覆盖 (`src/api/client.ts:2`)。
...
- 推荐方式: **一个 GPU 一个 `data_digging.py` 进程 + `--in-process` 顺序执行**, 用 `CUDA_VISIBLE_DEVICES=$i` 与 `nohup` 后台启动, 8 片环境集合互不重叠且并集 = 16 组合, 每卡 168 实验, 共享 `--batch-name full` (§2)。
...
- 赛道二: `CUDA_VISIBLE_DEVICES=0 python predict.py ... --target-input ... --target-epigenetics CTCF Dnase H3K4me3 RRBS` (§5)。
...
|---|---|---|---|---|
| `feature_schema.json` | `src/feature_engineering.py` / `backend_runner.run_feature_engineering_step` (`:107-117`) | `predict.main`, `data_digging.load_environment_combinations`, `train.py`, `cell_line_division.load_feature_schema` | `sequence_length, channel_count, feature_count, channel_names[8], sequence_channels` | 数据准备后长期存在; 预测/训练每次读 |
...
| `qc_summary.json` | 引擎 `analyse/data_QC.py` (`:1580`) | `qc_service.get`, `QcDashboard` | 引擎 QC 结构化结果 (`sample_size.total_samples/per_cell_line`, `length_consistency.recommended_length_L`, `ambiguous_bases.n_affected_sequences`, `outliers.n_outliers`, `qc_pass`) | QC 完成后 |
| `quality_report.md` / `figures/*.png` | `analyse/data_QC.py` (`:1577`) | `/api/artifacts`, `/api/artifact-raw`, QcDashboard | Markdown 报告; PNG 图 | QC 完成后 |
...
| `analysis_status.json` | `analyse/pipeline.py:261` (`ExecutionPlan.save_status`) | `analysis.analysis_status`, 前端轮询 | `plan_version, started_at, completed_at, tasks[{task_id,selected,available,status,reason}]` | 引擎写, GUI 只读 |
| `execution_log.json` | `analyse/pipeline.py:290, 302-323` | `analysis_status` 兜底判 completed | `executed{task→status}`, `figures[]` | 引擎写 |
| `analysis/{tables,summary,figures}/` | `PipelineArtifacts.create` (`analyse/pipeline.py:29-40`) | `analysis.list_outputs`, ReportsPanel, ArtifactViewer | 表格 csv / 报告 md / 图 png | 分析产物 |
...
| `results/<batch>/<run_name>/<model>_info.txt` + `_metrics.json` | `train.py` | `data_digging.parse_folder_info` → resume | `model/environment/combination/split_type/cell_line/random_seed/sequence_kernel/...` | 实验级 |
...
| `logs/workspace/backend.log` / `logs/workspace/frontend.log` | `scripts/run_workspace.sh` | 人工 | 服务端 stdout | 每次启动覆盖 |
...
|---|---|---|---|---|---|
| 1 | QC → Wizard/Frontend | 用户给定 CSV/TSV/目录 `input_paths` | `session_manifest.json`(status) + `qc_summary.json` + `quality_report.md` + `figures/*.png` | `StandaloneQcView` / `WorkspaceView` → `POST /api/qc/sessions` | `qc_service.QCSessionManager` → 子进程 `analyse/data_QC.py --data <f> --output-dir <session>` |
...
| 3 | Wizard → Training | `form_data` 勾选 (细胞系/表观/模型/划分) | 7 步流水线: 特征工程 → `data_digging.py` CLI → (可选) `predict.py` CLI → analyse 4 脚本 → plots → 交付物核对 | `main_wizard._run_backend_task` | `Input/backend_runner.execute_full_pipeline` → `subprocess.run` |
| 4 | Training → Analyse | `results/<batch>/<run_dir>/{<model>_info.txt,<model>_metrics.json,...}` | `results/<batch>/summary/{metrics_tables/*.csv, feature_importance/*, anomaly_report.md, plots/*}` | `backend_runner` Step 3-6; 或 `AnalysisPanel` 指定 batch 路径 | `analyse/collect_results.py`, `anomaly_treatment.py`, `importance_extraction.py`, `visualization.generate_all_visualizations` |
| 5 | Workflow → Analyse | `batch_dir` + `selected_tasks[]` (+ `project_id`/`output_dir`) | `analysis_plan.json` → `analysis_status.json`/`execution_log.json` + `tables/summary/figures` | `POST /api/analysis/run` | `analysis.build_plan_dict` → 子进程 `python -m analyse.pipeline --batch-dir --output --analysis-plan` |
...
| 11 | Grid → train.py | `Experiment` 元组 | `results/<batch>/<run_name>/…` + `logs/` + `models/` | `data_digging.main` | `train.py` (子进程或 in-process) |
...
| 入口 | 命令 | 关键事实 |
|---|---|---|
| Full workflow GUI (本地 Web) | `bash scripts/run_workspace.sh` | 后端 `PYTHONPATH=<repo>/backend python3 -m crispr_workspace.server --port 8765 --root $WS_ROOT` (日志 `logs/workspace/backend.log`); 前端 `npm run dev -- --port 5173` (node_modules 缺失时自动 `npm install`); 打开 `http://127.0.0.1:5173` (`run_workspace.sh:1-27`) |
| 后端单独启动 | `PYTHONPATH=backend python3 -m crispr_workspace.server --port 8765 [--root DIR]` | 默认 root `<repo>/workspace`; 可用 `CRISPR_WORKSPACE_ROOT` (`backend/README.md:8-14`, `server.py:270-279`) |
| 前端构建/类型检查 | `cd frontend && npm run build` / `npm run typecheck` | `tsc --noEmit -p tsconfig.json && vite build` (`package.json:8-10`); `frontend/dist` 当前不存在 |
...
| 桌面 7 步向导 (无代码) | `python Input/main_wizard.py` | 默认输出根 `<repo>/Project_Output`; 后台线程调 `execute_full_pipeline` (`README.md:121`, `main_wizard.py:534-536`) |
| 特征工程 | `python src/feature_engineering.py` (**README 写的 `python feature_engineering.py` 不存在**) | 生成 `(N,23,8)` 与 `feature_schema.json` |
| grid training | `python data_digging.py --batch-name <b> --models ... --cell-lines ... --split-types single all mixed [--training-scope-epis ... \| --environments ...] [--in-process] [--workers N] [--dry-run]` | 源: `README.md:140`, `HPC_EXPERIMENT_PROTOCOL.md:43-56`; 并发语义见 §15.8 |
| Ultimate predict (含断点续跑) | `python predict.py --batch-name <b> --data-dir data/proceeded_data --results-dir results --models linear xgboost mlp transformer cnn --cell-lines ... --target-input <csv> --target-epigenetics CTCF Dnase H3K4me3 RRBS` | 中断后重跑同一命令 → 只补缺完成标记的模型 (`predict.py:819-820,845-849`) |
| analyse | `python analyse/collect_results.py --batch-dir results/<b> --split-types single all mixed`; `analyse/anomaly_treatment.py --batch-dir …`; `analyse/importance_extraction.py --batch_dir …`(下划线); `analyse/visualization.py --batch-dir …`(连字符) | 参数风格不统一, 见 §23 |
...
| Workflow Analyse 子进程 | `python -m analyse.pipeline --batch-dir <b> --output <out> --analysis-plan <plan.json>` | `analysis.py:202-204` |
| 数据 QC 引擎 (headless) | `python analyse/data_QC.py --data <csv|dir> --output-dir <dir> [--print-markdown]` | `data_QC.py:1717-1721` |
| tests (后端) | `cd backend && PYTHONPATH=. python3 -m unittest tests.test_workspace -v` (README); 全套 = `backend/tests/{test_workspace,test_workflow_interfaces,test_e2e_smoke,test_project_recovery,test_no_training_dependency}.py` | 实测 19 个 `def test_`; E2E smoke 含真实 QC + analyse.pipeline(合成小批次) + artifact 读取; 训练仅 dry-run |
| 打包上传 | `bash scripts/build_upload.sh [out.tar.gz]` | 排除 `_no_upload/__pycache__/*.pyc/.npm_tmp/frontend/node_modules/frontend/dist/.venv/.git` (`build_upload.sh:7-15`) |
| HPC 全量 | 见 `HPC_EXPERIMENT_PROTOCOL.md` §2 (8×`CUDA_VISIBLE_DEVICES=$i nohup python data_digging.py … --in-process`) | 引用的 `HPC_launch_full_batch.sh` 不在仓库 |
...
| # | 位置 | 文档/注释说法 | 代码事实 | 判定 |
|---|---|---|---|---|
| 1 | `data_digging.py:404-405` vs `:146-162` | 暴露 `--mixed-seeds` / `--cnn-kernels` 参数 | `main()` 调 `generate_experiments` 时**未传**这两项, 函数内部直接用模块常量 `MIXED_SEEDS`/`CNN_KERNELS` → 两个 CLI 参数完全无效 | **Unused / 参数失效** |
| 2 | `predict.py:752-753` | `--generate-candidates` "(兼容旧参数, 已无意义)" | 除 `add_argument` 外全仓库无引用 | **Unused** (代码自认) |
| 3 | `predict.py:476-559 / 651-716` | — | `train_ultimate_models`、`generate_track2_results_ultimate` 无调用点; `main()` 走 `run_ultimate_with_resume` | **Legacy** |
...
| 10 | `README.md:135` Step 0 `python feature_engineering.py` | 根目录无此文件, 实际为 `src/feature_engineering.py` | **文档路径过期** |
| 11 | `README.md:174` 方式 3 引用 `notebooks/01_pipeline_demo.ipynb` | 仓库无 `notebooks/` 目录 | **文档引用缺失产物** |
| 12 | `backend/README.md:17` | 接口见 `docs/frontend_architecture.md` §4 接口表 | 实际接口表在 §3 | **交叉引用错位 (轻微)** |
...
| 15 | `frontend/src/api/qc.ts:9-13` `reuseCheck`, `api/artifacts.ts:24-30` `asCsv`, `api/analysis.ts:30-31` `plan` | 客户端方法已定义 | 组件层零调用 (`grep` 验证) → 对应后端路由 11/21 无前端消费者; `asCsv` 亦无用 | **Unused** |
...
| 21 | `frontend/src/views/StandaloneQcView.tsx:4` | import `ArtifactViewer` | 该视图未渲染 `ArtifactViewer` (仅 QcDashboard) | **未使用 import** |
...
| 23 | `analysis.py:174-178` | 仅对有 `enabled` 的容器 (environment/sequence/cell_line) 设置 `enabled` | 与 `analyse/plans.py:54-65` (StatisticsPlan/EvidencePlan 确无 `enabled`) 一致; `task()` 对 statistics/evidence 直接读子字段 (`plans.py:186-192`) | **一致 (易误改点, 已在注释标注)** |
| 24 | `scripts/run_workspace.sh` | 前端 `npm run dev` | `frontend/dist` 不存在, 且 `server.py` **不提供静态文件服务** → 无 dev server 时 Web GUI 不可用 (与 DSH `dsh web` GUI 无关, 属另一服务) | **部署事实** |
| 25 | `analyse/registry.py:52-76` | 15 条内置任务注册; runner 全为 `None` | `analysis.registry_tasks()` 回退表含 16 条 (额外 `fdr_correction`), 仅当 `import analyse.registry` 失败时才使用 → 正常路径任务数 = 15 | **回退表与注册表条目数不一致** |

---

## 本节自检

- **predict resume**: 覆盖 `PROGRESS_FILE`/`_load_progress`/`_save_progress`/`_kind_done`/`preds_<kind>.npy` 原子写/`run_ultimate_with_resume` 跳过逻辑, 并明确"不扫描 results/batch 目录"(`predict.py:819-820, 845-849, 873-877, 931-1003`, §15.5) ✅
- **data_digging resume**: 覆盖 `parse_folder_info`/`build_completed_lookup`/`classify_experiments` 的 key 规则与 `*info*.txt`+`*metrics*.json` 扫描依据 (`data_digging.py:183-247`, §15.8) ✅
- **wizard steps**: 7 步逐条给出标题、form_data 键、控件联动与校验 (`Input/main_wizard.py:109-427`, §16.2–16.4) ✅
...
## 24.1 运行顺序主链（call chain，实测）
```text
Input/main_wizard.py (GUI)
  └── Input/backend_runner.py::execute_full_pipeline(form_data, root_output_dir)
        ├── run_feature_engineering_step()
        │     └── convert_raw_csv_to_npy()  或 复制基准 npy
        ├── data_digging.py  (subprocess)
        │     └── train.py::main()  (subprocess 或 --in-process 时 import train 后调用 main)
        ...
        ├── predict.py  (仅 has_target; subprocess)
        │     └── run_ultimate_with_resume()
        │           ├── _run_ultimate_cv()/_fit_model()/_predict_model()
        │           └── _write_track2_from_preds()
        ├── analyse/collect_results.py (subprocess)  → summary/metrics_tables/
        ├── analyse/anomaly_treatment.py (subprocess) → summary/anomaly_report.md
        ├── analyse/importance_extraction.py (subprocess) → summary/feature_importance/
        └── analyse/visualization.py::generate_all_visualizations()  (经包兼容桥 import)
              → summary/plots/

analyse/pipeline.py::run_analysis()
  ├── analyse/data/loaders.load_experiment_table()
  ├── analyse/data/validation.validate_metric_consistency()
  ├── analyse/environment/incremental_effect.*
  ├── analyse/attribution/extractors.extract_attribution_table()
  ├── analyse/cellline/consistency.*
  ├── analyse/evidence/integration.environment_evidence_matrix()
  ├── analyse/reports/markdown_report.build_*()
  └── analyse/visualization.render_all()

Scientific Workspace:
frontend (React) ──HTTP──> backend/crispr_workspace/server.py::Handler
   ├── project.*        (manifest/CRUD/from-qc/stage)
   ├── qc_service.QCSessionManager ──subprocess──> analyse/data_QC.py
   ├── training.TrainingConfig.build_command() ──> data_digging.py / predict.py
   │     └── LocalRuntime/HpcRuntime (training_status.json)
   ├── analysis.registry_tasks/build_plan_dict/run ──subprocess──> python -m analyse.pipeline
   └── artifacts.resolve_artifact()（白名单 + CSV 分页）
```

## 24.2 import 依赖（静态，节选）
```text
analyse/pipeline.py imports:
...
backend/crispr_workspace/server.py imports:
  .{analysis, artifacts, config, project, qc_service, store, training}
backend/crispr_workspace/analysis.py:
  运行期 try-import `analyse.registry`（仅取任务目录；科学计算仍走子进程）
src/* 模型模块:
  各自 import `src.xai_importance`（白名单列/导出）
train.py:
  用 importlib 按 MODEL_MODULES 字符串动态 import 模型模块（因此静态 import 图上出现“无入度”的模块，如 src/xgboost/xgboost.py）
```
> **重要区分**：`backend_runner.py` 通过**子进程字符串路径**调用 `data_digging.py/predict.py/collect_results.py/anomaly_treatment.py/importance_extraction.py`，
> 因此这些模块在静态 import 图中“入度=0”，但它们都是 Active（有真实运行调用）。
> 同理 `analyse/visualization.py` 通过包 `__getattr__` 的 importlib 桥被调用。

## 24.3 静态“无项目内 import”但 Active 的模块（实测）
`Input/main_wizard.py`、`Input/backend_runner.py`、`predict.py`、`data_digging.py`、`train.py`、`scripts/make_notebook.py`、
`src/feature_engineering.py`、`src/xgboost/xgboost.py`（动态 import）、
`analyse/{data_QC,collect_results,importance_extraction,anomaly_treatment,visualization}.py`（子进程/桥）、
`backend/crispr_workspace/*`（由 server/CLI 启动）。
...
| 文件 | 状态 | 判定依据 |
|---|---|---|
| `train.py`, `predict.py`, `data_digging.py`, `src/feature_engineering.py` | Active（入口） | CLI 入口，被文档/GUI/Workflow 调用 |
| `src/{linear_regression,xgboost,mlp,cnn,transformer}/*.py`, `src/xai_importance.py`, `src/input_control/*.py` | Active（训练系统） | 被 train.py/各模型引用 |
| `analyse/{data_QC,collect_results,importance_extraction,anomaly_treatment,visualization}.py` | Active（训练侧独立脚本） | 被 `Input/backend_runner.py` 以子进程/桥调用；`HPC_EXPERIMENT_PROTOCOL.md` 亦引用 |
| `analyse/` 新引擎（pipeline/config/schemas/plans/registry/data/stats/environment/attribution/cellline/evidence/reports/visualization 包） | Active（分析引擎） | 被 pipeline/backend.analysis/tests 引用 |
| `backend/crispr_workspace/*` | Active（Workflow 后端） | 被 server/CLI/前端/测试引用 |
| `frontend/src/**` | Active（前端） | vite 构建入口 `main.tsx` → `App.tsx` |
| `analyse/visualize_results.py` | **工作区已删除** | `ls` 不存在；git status 标 `D` |
...
## 25.2 函数级残留（示例）
- `predict.py::train_ultimate_models` / `generate_track2_results_ultimate`：**Unused**（`main()` 已改用 `run_ultimate_with_resume`；grep 确认无调用）。
- `analyse/stats/hypothesis_tests.py::anova_interface`：**Planned/Placeholder**（返回 `available=False`）。
- `analyse/analysis.py`（engine）任务 `motif_discovery`：**Planned**；`bootstrap/hypothesis_testing/fdr_correction`：runner 未挂载（`unavailable`）。
- `analyse/registry.py` 中登记但未实现的任务：以 pipeline 运行时的 status/reason 为准（不伪造）。
...
| `build_command` | `data_digging.py` vs `backend/crispr_workspace/training.py` | 两套 CLI 拼装（前者直连 train.py，后者编排 data_digging/predict） |
...
3. **QC（只读）**：`analyse/data_QC.py` 体检（长度/歧义碱基/缺失/GC/离群），产出 `qc_summary.json`、`quality_report.md`、图；不修改数据；决策由 Wizard/Workflow 记录配置。
4. **训练配置**：GUI Step5/Workflow `TrainingConfig`（models/cells/splits/environments/超参/runtime）。
5. **网格实验**：`data_digging.py` 生成实验清单（single/all/mixed × 环境组合 × 模型 × CNN 核），逐实验调用 `train.py`；支持断点续跑与 8 卡并行。
6. **单实验训练**：`train.py` 划分数据（single/all/mixed）→ 环境组合掩码 → 模型训练/早停/测试 → 指标与 XAI 落盘。
7. **XAI**：linear 系数+统计检验、XGB Gain/Weight/Cover/TreeSHAP、MLP IG/SmoothGrad、CNN IG/ISM、Transformer Attention/熵，并计算 SNR；白名单列由 `src/xai_importance.py` 定义。
8. **结果收集**：`collect_results.py` 扫描实验目录 → `metrics_tables/all_experiments.csv` 等；`importance_extraction.py` 汇总重要性报告与特征库。
9. **结果级 QC**：`anomaly_treatment.py` 两级异常检测报告；`analyse/data/validation.py` 同 cohort 指标一致性标记。
...
12. **HPC**：`CUDA_VISIBLE_DEVICES` + 每卡一个 `data_digging.py` 进程 + `--in-process`，共享 batch 断点续跑；`predict.py` 支持模型级续跑。
...
|---|---|---|
| 数据与预处理 | `src/feature_engineering.py`, `src/input_control/*` | 23nt；4 碱基 One-Hot；4 表观通道；去重；schema 化 |
| 数据体检/质控 | `analyse/data_QC.py` | 只读检测；`qc_summary.json` 字段；离群 IsolationForest；阈值集中 |
...
| 训练协议 | `train.py`, `data_digging.py` | 实验网格；早停 patience/min_delta；checkpoint；日志 |
| 可解释性 | `src/xai_importance.py` + 各模型 XAI 函数 | 方法清单、SNR 定义、白名单列 |
| 实验规模与复现 | `data_digging.py`, `HPC_EXPERIMENT_PROTOCOL.md` | 1344 实验构成；8×A100 分片；`--in-process` |
| 统计与证据 | `analyse/` 引擎（config/stats/environment/evidence） | 配对基线原则、FDR family、Bootstrap、Evidence Tier、不稳定隔离 |
| 结果与报告 | `analyse/reports`, `analyse/visualization`, `frontend` | summary 00–08、figures、Artifact Viewer |
...
4. **网格里的 seed**：`data_digging.generate_experiments()` 中 `single`/`all` 使用 train.py 默认 seed（42）；
   `mixed` 使用 `MIXED_SEEDS = [42, 43, 44, 45]` 四次重复（`train.py::build_command` 对 mixed 传对应 seed）。
5. **标准化开关默认关闭**：`--use-scaler`（train.py / data_digging 默认 False）；
...
7. **⚠️ 关键缺陷：网格中的 `all` 划分实际退化为 `single`（已硬证据确认）**
   证据链（全部实测）：
   - `data_digging.py::build_command` 只传 `--cell-line <cl>`，不传 `--cell-lines`；
   - `train.py:663-664`：`if cell_line and not cell_lines: cell_lines = [cell_line]`；
   - `src/input_control/cell_line_division.py::divide_data` 把 `cell_lines` 过滤为可用集合（此处只有 1 个）；
   ...
   数据划分与随机种子。实测 `results/batch_20260909_full/` 中 **8/8** 组 (mlp/xgboost/transformer/cnn ×
...
|---|---:|---|---|---:|
| `data_digging.py` | 518 | Active (入口) | 已测数据 Training Scope 网格实验引擎 | 15 |
| `scripts/make_notebook.py` | 320 | Active (入口) | 生成 notebooks/01_pipeline_demo.ipynb 的脚本 | 0 |
| `predict.py` | 1008 | Active (入口) | mixed 十折 CV + 目标集预测 (Ultimate, 含断点续跑) | 29 |
| `train.py` | 734 | Active (入口) | 单次实验训练与评估入口 (dispatching 五模型) | 20 |
| `Input/backend_runner.py` | 306 | Active | 向导后端全流程调度 (Step1–7) | 4 |
| `Input/main_wizard.py` | 536 | Active (入口) | 7 步 Tkinter 引导向导 | 13 |
| `analyse/__init__.py` | 16 | Active (分析引擎) | analyse — CRISPR 编辑效率影响因素发现与证据整合分析引擎。 | 0 |
| `analyse/anomaly_treatment.py` | 489 | Active (训练侧独立脚本, 由 Input/backend_runner.py 以子进程调用) | 两级异常检测与 anomaly_report.md | 12 |
| `analyse/collect_results.py` | 442 | Active (训练侧独立脚本, 由 Input/backend_runner.py 以子进程调用) | 实验目录扫描 → metrics_tables 汇总 | 20 |
| `analyse/config.py` | 65 | Active (分析引擎) | analyse.config — 集中科学规则配置 (阈值不得散落各模块)。 | 1 |
| `analyse/data_QC.py` | 1768 | Active (训练侧独立脚本, 由 Input/backend_runner.py 以子进程调用) | 数据体检 (只读 QC, qc_summary.json/报告/图) | 36 |
| `analyse/importance_extraction.py` | 908 | Active (训练侧独立脚本, 由 Input/backend_runner.py 以子进程调用) | 各模型重要性白名单 → feature_importance md/CSV + 特征库 | 22 |
| `analyse/pipeline.py` | 345 | Active (分析引擎) | 分析引擎编排入口 (python -m analyse.pipeline) | 4 |
| `analyse/plans.py` | 242 | Active (分析引擎) | analyse.plans — AnalysisPlan / Validator / ExecutionPlan / 状 | 8 |
| `analyse/prediction.py` | 35 | Active (分析引擎) | analyse.prediction — 预测/泛化性能分析 (只读统一表)。 | 2 |
| `analyse/registry.py` | 77 | Active (分析引擎) | analyse.registry — 分析任务注册表 (新增分析 = 注册任务, 不改 GUI/核心流程)。 | 3 |
| `analyse/schemas.py` | 200 | Active (分析引擎) | analyse.schemas — 统一科学记录类型与证据标签。 | 0 |
| `analyse/visualization.py` | 959 | Active (训练侧独立脚本, 由 Input/backend_runner.py 以子进程调用) | V6 显著性热图/环境增量树/表观对比 (旧单文件) | 25 |
| `src/feature_engineering.py` | 2029 | Active (入口) | 原始 CSV → 23×C 特征矩阵/schema/npy | 27 |
| `src/xai_importance.py` | 135 | Active (训练系统) | XAI 特征重要性输出【白名单】与导出清洗 (学术红线) | 2 |
| `analyse/attribution/__init__.py` | 5 | Active (分析引擎) | analyse.attribution — 模型专属 attribution 统一抽取 (线性/树/深度)。 | 0 |
| `analyse/attribution/columns.py` | 61 | Active (分析引擎) | analyse.attribution.columns — 各模型重要性 CSV 列 -> 统一字段 的映射 (adap | 2 |
| `analyse/attribution/extractors.py` | 139 | Active (分析引擎) | analyse.attribution.extractors — 从训练产物抽取统一 attribution 表。 | 3 |
| `analyse/attribution/summary.py` | 50 | Active (分析引擎) | analyse.attribution.summary — 统一 attribution 表 -> 04 sequenc | 2 |
| `analyse/cellline/__init__.py` | 1 | Active (分析引擎) | analyse.cellline — feature × cell-line 效应与一致性。 | 0 |
| `analyse/cellline/consistency.py` | 60 | Active (分析引擎) | analyse.cellline.consistency — cell-line effect 汇总与 context  | 2 |
| `analyse/data/__init__.py` | 1 | Active (分析引擎) | analyse.data — 批量结果只读加载/字段适配/校验 (训练系统只读)。 | 0 |
| `analyse/data/loaders.py` | 173 | Active (分析引擎) | analyse.data.loaders — 从训练产物建立统一实验表 (只读, 不修改原始结果)。 | 8 |
| `analyse/data/validation.py` | 63 | Active (分析引擎) | analyse.data.validation — 结果校验。 | 2 |
| `analyse/environment/__init__.py` | 1 | Active (分析引擎) | analyse.environment — 环境因素效应分析 (条件 ΔR² / 主效应 / 交互占位)。 | 0 |
| `analyse/environment/incremental_effect.py` | 170 | Active (分析引擎) | analyse.environment.incremental_effect — 条件 ΔR² / 主效应 (Paire | 7 |
| `analyse/evidence/__init__.py` | 1 | Active (分析引擎) | analyse.evidence — Evidence Strength/Tier/假设语言规则。 | 0 |
| `analyse/evidence/hypothesis.py` | 74 | Active (分析引擎) | analyse.evidence.hypothesis — 生物假设语言 (受控措辞)。 | 1 |
| `analyse/evidence/integration.py` | 194 | Active (分析引擎) | analyse.evidence.integration — 跨模型/跨 cell-line 证据整合 (纯函数)。 | 8 |
| `analyse/evidence/rules.py` | 73 | Active (分析引擎) | analyse.evidence.rules — Evidence 标签规则 (阈值一律来自 config, 不散落)。 | 4 |
| `analyse/reports/__init__.py` | 1 | Active (分析引擎) | analyse.reports — Markdown/表格 生成 (统一引用统一数据, 不自行扫描)。 | 0 |
| `analyse/reports/markdown_report.py` | 180 | Active (分析引擎) | analyse.reports.markdown_report — summary/*.md 生成 (Phase 2 切 | 10 |
| `analyse/stats/__init__.py` | 2 | Active (分析引擎) | analyse.stats — Effect/CI/检验/FDR/Bootstrap 规范实现 (无副作用纯函数; 名  | 0 |
| `analyse/stats/bootstrap.py` | 107 | Active (分析引擎) | analyse.stats.bootstrap — Bootstrap CI (仅稳定性证据, 非因果/显著)。 | 3 |
| `analyse/stats/effect_size.py` | 32 | Active (分析引擎) | analyse.stats.effect_size — Effect 定义与 paired-baseline 原则。 | 2 |
| `analyse/stats/hypothesis_tests.py` | 76 | Active (分析引擎) | analyse.stats.hypothesis_tests — 归因统计检验接口。 | 2 |
| `analyse/stats/multiple_testing.py` | 35 | Active (分析引擎) | analyse.stats.multiple_testing — BH-FDR (Benjamini-Hochberg) | 1 |
| `analyse/tests/__init__.py` | 0 | Test-only | — | 0 |
| `analyse/tests/test_attribution.py` | 78 | Test-only | analyse.tests.test_attribution — attribution 统一抽取测试。 | 4 |
| `analyse/tests/test_core.py` | 201 | Test-only | analyse.tests — 核心科学函数测试 (unittest, 无 pytest 依赖)。 | 20 |
| `analyse/tests/test_environment.py` | 70 | Test-only | analyse.tests.test_environment — 条件 ΔR² / 主效应 (Paired baseli | 5 |
| `analyse/tests/test_phase5.py` | 63 | Test-only | analyse.tests.test_phase5 — cell-line 一致性 + evidence matrix  | 4 |
| `analyse/tests/test_phase6.py` | 110 | Test-only | analyse.tests.test_phase6 — pipeline 端到端报告/图形产物 (Phase 6)。 | 2 |
| `analyse/visualization/__init__.py` | 84 | Active (分析引擎) | analyse.visualization — 证据引擎图表 (分主题模块; 300dpi PNG)。 | 3 |
| `analyse/visualization/attribution_plots.py` | 38 | Active (分析引擎) | attribution_plots — 位置归因热图 (per method; magnitude)。 | 1 |
| `analyse/visualization/cellline_plots.py` | 41 | Active (分析引擎) | cellline_plots — feature × cell-line 效应与一致性。 | 1 |
| `analyse/visualization/core.py` | 21 | Active (分析引擎) | analyse.visualization.core — 图表样式/保存基元。 | 2 |
| `analyse/visualization/environment_plots.py` | 43 | Active (分析引擎) | environment_plots — 条件 ΔR² 热图 + 主效应。 | 1 |
| `analyse/visualization/evidence_plots.py` | 36 | Active (分析引擎) | evidence_plots — 证据矩阵/层级汇总。 | 1 |
| `analyse/visualization/performance_plots.py` | 35 | Active (分析引擎) | performance_plots — 预测/泛化图。 | 1 |
| `backend/crispr_workspace/__init__.py` | 11 | Active (Workflow 后端) | crispr_workspace — CRISPR Scientific Workspace 本地后端 (Workflo | 0 |
| `backend/crispr_workspace/analysis.py` | 256 | Active (Workflow 后端) | Analysis / AnalysisPlan 接口。 | 7 |
| `backend/crispr_workspace/artifacts.py` | 158 | Active (Workflow 后端) | Artifact Resolver / 预览服务。 | 5 |
| `backend/crispr_workspace/config.py` | 48 | Active (Workflow 后端) | 配置与路径策略 (跨平台, 无硬编码实验室路径)。 | 5 |
| `backend/crispr_workspace/fingerprint.py` | 61 | Active (Workflow 后端) | 数据指纹: 判断“当前输入 == 上次 QC 的输入”, 防止错误复用旧 QC。 | 3 |
| `backend/crispr_workspace/project.py` | 136 | Active (Workflow 后端) | Project Workspace: 创建/列出/打开项目, project_manifest.json 为状态源。 | 10 |
| `backend/crispr_workspace/qc_service.py` | 141 | Active (Workflow 后端) | Standalone / Workflow Data QC 会话管理。 | 9 |
| `backend/crispr_workspace/server.py` | 279 | Active (入口) | 本地 Workflow API (/api/*) | 9 |
| `backend/crispr_workspace/store.py` | 41 | Active (Workflow 后端) | 轻量 JSON 状态存取 (manifest/status 一律落盘, 前端状态从文件恢复)。 | 3 |
| `backend/crispr_workspace/training.py` | 494 | Active (Workflow 后端) | Training / Runtime Adapter 接口。 | 29 |
| `backend/tests/test_e2e_smoke.py` | 127 | Test-only | 端到端 smoke (无 GPU): QC(真实引擎) → 训练(校验+preflight+dry-run) | 3 |
| `backend/tests/test_no_training_dependency.py` | 42 | Test-only | 静态守卫: Workflow 后端绝不 import 训练/引擎科学模块 (红线 P0)。 | 1 |
| `backend/tests/test_project_recovery.py` | 54 | Test-only | Project recovery 端到端: 阶段/详情落盘后重开 (新实例) 必须可恢复。 | 2 |
| `backend/tests/test_workflow_interfaces.py` | 131 | Test-only | Workflow 接口测试: TrainingConfig/CLI 构建、Preflight、Runtime(dry-r | 12 |
| `backend/tests/test_workspace.py` | 157 | Test-only | crispr_workspace 后端单测 (仅标准库 + 可选的 analyse/data_QC 引擎)。 | 11 |
| `src/cnn/cnn.py` | 873 | Active (训练系统) | CNN | 13 |
| `src/input_control/cell_environment_combination.py` | 1908 | Active (训练系统) | Cell Environment Combination | 23 |
| `src/input_control/cell_line_division.py` | 334 | Active (训练系统) | Cell Line Division (动态数据发现与零硬编码版) | 15 |
| `src/linear_regression/linear_regression.py` | 852 | Active (训练系统) | Linear Regression Model | 14 |
| `src/mlp/mlp.py` | 801 | Active (训练系统) | MLP | 14 |
| `src/transformer/transformer.py` | 805 | Active (训练系统) | Transformer | 17 |
| `src/xgboost/xgboost.py` | 694 | Active (训练系统) | XGBoost | 14 |
...
- `L418` **extract_mlp_importance(target_batch_dir, feature_importance_dir)** — MLP —— XAI 白名单 (MLP_IG/SmoothGrad/IG_SNR), 无 p/FDR。
...
- `L245` **compute_smoothgrad(model, X, device, n_samples, noise_level)** — 原生 PyTorch 实现 SmoothGrad (平滑梯度均值与方差)
...
- `L422` **compute_and_save_shap(model, X_background, X_explain, feature_names, result_dir, device, background_samples)**
- `L481` **train(X_train, y_train, X_test, y_test, X_valid, y_valid, feature_names, run_name, model_dir, result_dir, log_dir, config, random_seed, epochs, batch_size, learning_rate, hidden_dim1, hidden_dim2, dropout, weight_decay, patience, min_delta, num_workers, device, use_scaler, compute_shap, shap_background_samples, shap_explain_X)**
- class **MLPModel** (L95)
