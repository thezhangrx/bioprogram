...
批次: `results/batch_20260909_full` (1344 experiments, 16 environment combinations, 7 model variants)
...
```bash
# 统计接线 (默认 ANOVA 关闭)
python -m analyse.pipeline --batch-dir results/batch_20260909_full

# 打开 ANOVA
python -m analyse.pipeline --batch-dir results/batch_20260909_full --analysis-plan plan_anova_on.json
...
| 能力 | 实现 |
| :--- | :--- |
| 报告数据层 | `frontend/src/reports/ReportDataAdapter.ts` (唯一数据入口: CSV/MD/JSON -> ReportData) |
...
| 测试 | `frontend/src/reports/ScientificReport.test.ts` (9 例) + 全量 vitest 16 例 |
...
| 测试 | `analyse/tests/test_importance_delta_r2.py` (16 项) |
