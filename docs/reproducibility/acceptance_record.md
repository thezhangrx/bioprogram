...
| 项 | 结果 |
|---|---|
| 后端 unittest (`app/backend/tests`) | **19/19 OK** (workspace / workflow interfaces / 真实 E2E smoke / project recovery / 零训练依赖静态守卫) |
| 前端 vitest | **4 文件 / 7 tests OK** (format、artifact 路径解析、StatusChip、EvidenceBadge) |
| 前端 `tsc --noEmit` + `vite build` | OK (54 模块, ~58KB gzip) |
| `app/scripts/run_workspace.sh` 冒烟 | backend `/api/health` OK · frontend HTTP 200 |
| E2E smoke 内容 | 真实 QC(引擎)→ 训练校验/preflight/dry-run → 真实 `analyse.pipeline`(合成小批次) → 报告 Artifact 读取 |

## 2. 交付物盘点 (对应需求 §44)
1. **新前端目录树** — `docs/frontend_architecture.md` §2; `app/frontend/` (api 6 客户端, components 10+, views 4)。
2. **页面/Component 结构** — Home(双入口+Projects)/CreateProject/Workspace(侧栏+Notebook Cell)/StandaloneQC。
3. **API 接口定义** — `app/backend/crispr_workspace/server.py` 路由 + `app/frontend/src/api/*.ts`; §3 总表。
...
12. **测试结果** — 上表; `app/backend/tests/*`, `app/frontend/src/**/*.test.ts(x)`。
...
## 4. 需在目标机器上完成的用户侧步骤
- 上传: `bash deploy/hpc/build_upload.sh /tmp/Submit_upload.tar.gz` → scp → 解压到 `Submit/`。
- 启动: `conda activate <env>; bash app/scripts/run_workspace.sh` → 浏览器 `http://127.0.0.1:5173`。
